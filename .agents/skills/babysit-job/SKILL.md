...
1. `job_id` — Iris job ID in canonical format `/<user>/<job>` (e.g., `/timodonnell/iris-run-train_protein_30m-20260512-185630`).
2. `config` — Iris config path. MarinFold does not vendor iris configs into the
...
3. `resubmit_command` — exact Iris submit command for resubmission; **must
   include `--no-wait`**.
...
- Cross-region GCS I/O latency: if checkpoint writes look slow, confirm the
  TPU zone matches the bucket region (`us-east5-a` for `marin-us-east5`). See
  `models/AGENTS.md` "Co-locate TPU with checkpoint bucket".
- Tokenizer cache poisoning: a sudden `vocab_size` mismatch usually means the
...
- Debug running tasks with `iris task exec` → **debug-iris-job** (not yet ported).
