...
3. **Compute and GPU scope.** Recommend an overall GPU limit from the training
   code and prior runs. Confirm whether to use both H100 and GB200, both H100
   clusters (`cw-us-east-02a` and `cw-rno2a`), and GB200 on
   `cw-us-east-08a`; recommend all three production clusters by default.
