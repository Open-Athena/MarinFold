...
revision floor. Record `client_floor_failed`, stop new submissions and recovery
stops, update only `BUILD_DATE` to the required floor, and prove a canary submission
is accepted. Otherwise stop and give the operator the dates and failure. Never pull,
upgrade, reinstall, or restart the shared Iris cluster autonomously.
