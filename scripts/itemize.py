...
"""``scripts/itemize.py`` — print the experiment index from gh + frontmatter.

The index is **rendered on demand and written nowhere**. It used to live at
``experiments/index.md``, but two of its columns (Branch, Directory) are read
from the working tree, so every branch generated a copy that differed from
main's in its own row — a guaranteed merge conflict once main moved. Nothing
linked to the file, and ``gh issue list --label experiment`` is the same index,
so it was retired rather than kept in sync. Pass ``--output`` if you want a
file anyway; don't commit it.
...
def render_index(
    ...
    lines = [
        ...
        "",
        f"_{total} experiments, rendered by `python scripts/itemize.py`._",
        "",
    ]
    for kind in KINDS:
        rows = by_kind[kind]
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        help="Write to this path instead of stdout. Ad-hoc only — the index is "
             "not a tracked file; don't commit what this writes.",
    )
    ap.add_argument(
        "--repo", default=None,
