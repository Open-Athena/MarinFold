...
"""``scripts/history.py`` — manage the persistent audit trail of W&B runs.
...
from _lib import REPO_ROOT, RUN_KINDS, parse_experiment_dir_name  # noqa: E402
...
def _existing_run_files() -> list[RunFile]:
    ...
    for p in sorted(RUNS_DIR.glob("*.md")):
        try:
            ...
            print(f"[history] WARN: skipping unparseable {p}: {exc}", file=sys.stderr)
    return out
...
def _cmd_new(args: argparse.Namespace) -> int:
    ...
    if args.wandb_name is None:
        print(
            ...
            "or use `python scripts/history.py sync` which fetches it from the API.",
            file=sys.stderr,
        )
        return 2
    ...
    if run_id in existing and not args.force:
        print(
            ...
            "Use --force to overwrite, or `python scripts/history.py add-iris-job` to append a job ID.",
            file=sys.stderr,
        )
        return 1
...
def _cmd_update_index(args: argparse.Namespace) -> int:
    ...
    if args.check:
        ...
        if current.strip() != content.strip():
            ...
            print("Regenerate with: python scripts/history.py update-index", file=sys.stderr)
            return 1
...
def _cmd_sync(args: argparse.Namespace) -> int:
    ...
    existing = _runs_by_id(_existing_run_files())
    print(
        f"[history] {len(existing)} existing run files; fetching W&B runs from "
        f"{entity}/{project}...",
        file=sys.stderr,
    )
    ...
    print(
        f"[history] sync done. saw {seen} W&B runs, created {created} new history files.",
        file=sys.stderr,
    )
    return 0
...
def _cmd_check(args: argparse.Namespace) -> int:
    ...
    if not missing:
        print(f"[history] OK — all {len(existing)} W&B runs have history files.")
        return 0

    print(
        f"[history] DRIFT — {len(missing)} W&B run(s) lack a history file:",
        file=sys.stderr,
    )
    for run_id, run_name in missing:
        print(f"  - {run_name} ({run_id})", file=sys.stderr)
    print("Run `python scripts/history.py sync` to create skeletons.", file=sys.stderr)
    return 1
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="python scripts/history.py",
