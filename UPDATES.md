...
* **Data: we are experimenting with a cheap way to expand our training set.** We use ProteinMPNN to generate new sequences for the same structures in our existing training set. This will give us additional sequence diversity, but not structural diversity. Combined with the original data, this gets us up to 232M documents and 248B tokens, a 3.3-fold increase from the previous training set. See ([#266](https://github.com/Open-Athena/MarinFold/issues/266), [#267](https://github.com/Open-Athena/MarinFold/pull/267)). AFDB — [contacts_v1_mpnn_redesign](https://huggingface.co/buckets/open-athena/MarinFold/tree/data/document_structures/contacts_v1_mpnn_redesign), **31.7M documents / 35.32B tokens**, 8 designs per backbone. ESM-Atlas — [contacts_v1_esm_atlas_mpnn_redesign](https://huggingface.co/buckets/open-athena/MarinFold/tree/data/document_structures/contacts_v1_esm_atlas_mpnn_redesign), **130.9M documents / 138.62B tokens**, 2 designs per backbone.
* **Training: I tried a variant where we get rid of RoPE and use "token smearing" to allow the model to track local token proximity.** This seemed like an intuitive idea since the order of contact statements in a document is random so it would make sense for the model to condition on it as a set rather than an ordered list of statements. It looked promising at small scale but turned out to not help at all in a full training run. ([#262](https://github.com/Open-Athena/MarinFold/issues/262), [#272](https://github.com/Open-Athena/MarinFold/pull/272)). Summary: replacing RoPE with a width-3 causal token smear costs **0.0275 nats** at the production 1.5B scale over the full 152B-token schedule (2.9745 → 3.0021); both arms completed all 145,200 steps.
* **Training: Zack ran an experiment (exp157) where he is setting the embeddings for the position tokens to fixed values.** I am actually not sure if this is just a different initialization or if they are fixed for the entire training run. It ran on our older AFDB-only 16-epoch recipe, so the results aren't comparable to our recent models (R-precision < 0.5 on eval-val for all runs). Let's discuss this experiment at the meeting. ([#157](https://github.com/Open-Athena/MarinFold/issues/157), [#268](https://github.com/Open-Athena/MarinFold/pull/268)).
* **Training: Zack is continuing to work on the soft-target loss training.** No training results yet. [#177](https://github.com/Open-Athena/MarinFold/issues/177), [#273](https://github.com/Open-Athena/MarinFold/pull/273).
* **Helico: better contacts convert straight into better structures** ([helico#17](https://github.com/Open-Athena/helico/pull/17)). Re-running only the MarinFold arm of helico's exp14 on the new default's step-363,000 contacts moves contact precision @L 0.510 → **0.542**, and lDDT **0.605 → 0.630** on eval-val and **0.619 → 0.642** on eval-test.
* **Housekeeping: switched the default model to be the one trained on decontaminated data** ([#270](https://github.com/Open-Athena/MarinFold/pull/270)). `contacts-v1-exp232-m2-p06-train-1.5B` (step 363,000) displaces the #199 cooldown: **R-precision 0.6051 legacy-554 / 0.5517 eval-val**, val loss 2.9681. The cooldown is still the highest scorer (0.631) but is now labelled CONTAMINATED in `MODELS.yaml`, as are the pre-[#225](https://github.com/Open-Athena/MarinFold/issues/225) entries; README, DOCS and the notebooks all move onto the new default.

### Upcoming

* Tim: experiment with an additional data generation strategy for increasing structural diversity in our training set. The idea is to use [Proteina](https://github.com/NVIDIA-BioNeMo/proteina) to generate new backbones. Still figuring out how expensive this will be / if we have enough compute for it.
* Tim: I want to return to post-training but this time focus on SFT rather than RL. My current idea is to continue the multiple-predictions-per-rollout idea, but rather than do it as online RL we just generate a much larger dataset using the current weights, fine tune on it, and repeat that a few times. The intuition here is that RL is only stable for a few hundred training steps, but I think we will need tens of thousands of training steps to get the model to learn the fundamentally new capability we are after (i.e. generating multiple structural hypotheses in a way that balances diversity and accuracy).
* Zack is going to launch [#274](https://github.com/Open-Athena/MarinFold/issues/274) where we train a new model on our 248B token training set using Eric's sweep configuration. We will not wait for Jacob's multimer data ([#145](https://github.com/Open-Athena/MarinFold/issues/145)) for this. Will be interesting to see if this gets us a new best model!
* Zack: soft-target training results ([#177](https://github.com/Open-Athena/MarinFold/issues/177) / [#273](https://github.com/Open-Athena/MarinFold/pull/273)).
