...
* **Eric's tuned models are a lot more accurate, not just lower-loss.** Re-scored our 554-protein contact benchmark: his current best 1.5B ([#117](https://github.com/Open-Athena/MarinFold/issues/117), eval loss 2.7037) gets **R-precision 0.53** vs **0.42** for the [#61](https://github.com/Open-Athena/MarinFold/issues/61) model we have been quoting (long-range 0.48 vs 0.37). Still short of the structure predictors on top-K (Protenix-v2 single-seq 0.60, ESMFold2 0.79) — but on **AUC we are at 0.932**, second only to Protenix-v2 with an MSA. So we rank the whole contact map about as well as a structure predictor; what we can't do is concentrate confidence into the top L pairs. [Plot](experiments/exp82_evals_contacts_v1_contact_prediction/plots/where_we_stand_rprecision.png), [details](experiments/exp82_evals_contacts_v1_contact_prediction/README.md).
* **All our rollout evals had `top_k=50` in them** — an inherited HuggingFace default baked into the checkpoint export, never a decision anyone made. It suppresses contacts (rollouts asserted 0.67× the true count with it, 0.96× without) and cost ~0.011 R-precision / 0.020 AUC. Now off by default; partly walks back the under-generation worry from [#142](https://github.com/Open-Athena/MarinFold/issues/142).
* **Of that 0.41 → 0.53, ~91% is the better model and ~7% the sampling fix** — measured as a full 2×2, not assumed. The stronger model already under-generated less, so it had less to gain from untruncating. AUC is the exception: there the sampling change is about a third of the gain.
* **The ESMFold2-Atlas corpus landed ([#139](https://github.com/Open-Athena/MarinFold/issues/139), [#141](https://github.com/Open-Athena/MarinFold/pull/141)) — 66.8M documents / 71.4B tokens,** ~16× the AFDB set and the "67M instead of 4M" expansion we have wanted since [#91](https://github.com/Open-Athena/MarinFold/issues/91). Thanks Jacob for the curation. We also published the raw pyconfind contacts, so the next document format over this source is a cheap serialization pass instead of ~2,850 core-hours. CoreWeave's CPU pool ran it in ~7h; the GCP pool would have taken ~37 days.
* **First training on it ([#155](https://github.com/Open-Athena/MarinFold/issues/155)):** a 1.5B on crops + contacts-v1 + ESM-Atlas, one epoch of each — 28% in at `contacts-v1-val` **2.809**.
* **Backtracking ([#158](https://github.com/Open-Athena/MarinFold/issues/158), [#159](https://github.com/Open-Athena/MarinFold/issues/159), [#160](https://github.com/Open-Athena/MarinFold/issues/160))** — the self-correction idea from last update. `<retract>` is implemented ([#161](https://github.com/Open-Athena/MarinFold/pull/161)); documents are byte-identical with it disabled and no existing token IDs move. The generator retracts when the model's *own* posterior turns against a contact it already emitted — no ground truth in the loop — and that caught **76.4% of its false positives while never once retracting a true contact**. 1.02M documents generated on 48 H100s in ~4.5h. Training next.
* **Rollout refinement ([#163](https://github.com/Open-Athena/MarinFold/issues/163), [#164](https://github.com/Open-Athena/MarinFold/pull/164)):** consensus voting over K rollouts is a dead tie with the model's own calibrated matrix (0.224 vs 0.221), so more rollouts buy nothing. But conditioning it on 50% of a protein's *true* contacts lifts R-precision on the rest from 0.145 to **0.556**, while a *noisy* rollout makes it worse. That gap is exactly what a refiner would have to learn. First training run started.
* **Eric is scaling tokens and parameters ([#154](https://github.com/Open-Athena/MarinFold/issues/154)):** 2.757 (1.5B/8ep, [#75](https://github.com/Open-Athena/MarinFold/issues/75)) → 2.713 (1.5B/8ep better tuning, [#117](https://github.com/Open-Athena/MarinFold/issues/117)) → 2.704 (1.5B/16ep) → 2.702 (3B/8ep, [#146](https://github.com/Open-Athena/MarinFold/issues/146)); 6B ([#153](https://github.com/Open-Athena/MarinFold/issues/153)) is blocked, the H100 setup looks broken. Note 3B/8ep ≈ 1.5B/16ep and the whole ladder spans only 0.055 nats — but 0.05 nats bought +0.11 R-precision, so the loss→accuracy slope has *not* flattened. He also opened [#166](https://github.com/Open-Athena/MarinFold/issues/166) (amino-acid augmentation from the best 8-epoch models vs random init).
* **Zack started a greedy latent contact-set loss ([#156](https://github.com/Open-Athena/MarinFold/issues/156), [#167](https://github.com/Open-Athena/MarinFold/pull/167))** — trains on the unordered contact set instead of penalizing arbitrary serialization order and orientation. Runs end-to-end in Levanter on H100s at ~95% GPU utilization, comparable to stock next-token CE; model-quality results are still inconclusive because the curves are noisy. GB200 runs blocked on an eval issue. He also opened [#157](https://github.com/Open-Athena/MarinFold/issues/157) (relative/RoPE-style residue-position embeddings).
* **Other training in flight:** crops + 5% contacts-v1 ([#137](https://github.com/Open-Athena/MarinFold/issues/137)) 69% through at contacts-v1-val 2.992; our **first straight reproduction of one of Eric's runs** ([#150](https://github.com/Open-Athena/MarinFold/issues/150)) 41% through, which will tell us whether our "data effects" in #120/#121/#137 are partly harness effects; on-the-fly document generation from the saved ESM-Atlas contacts ([#147](https://github.com/Open-Athena/MarinFold/issues/147)) 22% through.
* **A caveat on every eval loss in this doc:** the cached packed dataset scores `<pad>` positions, and **12.3% of our contacts-v1 validation positions are padding**. Comparisons between our runs are still valid; the absolute number is flattered. Found while chasing an unexplained gap in [#147](https://github.com/Open-Athena/MarinFold/issues/147).
* **Housekeeping:** nine bugs fixed in a codebase review ([#148](https://github.com/Open-Athena/MarinFold/pull/148), thanks Sankalp); an inference fix for checkpoints exported by newer transformers ([#165](https://github.com/Open-Athena/MarinFold/pull/165)); an [ESM-Atlas explorer Colab](https://colab.research.google.com/github/Open-Athena/MarinFold/blob/main/notebooks/explore_esm_atlas_distill.ipynb) ([#140](https://github.com/Open-Athena/MarinFold/pull/140)). Also: the "deleted" #117 checkpoints aren't deleted — they moved to `gs://<bucket>/checkpoints/protein/`, which unblocks the warm-starts in #160 and #163.

### Upcoming

* Train on the backtracking corpus ([#160](https://github.com/Open-Athena/MarinFold/issues/160)). The bar is high — #163 says voting already extracts everything the marginal has, so this has to reach the *joint* signal to be worth anything.
* Finish the [#163](https://github.com/Open-Athena/MarinFold/issues/163) refiner run and decide whether to scale its corpus from 10k to 1M proteins.
* Let the long runs finish ([#155](https://github.com/Open-Athena/MarinFold/issues/155), [#137](https://github.com/Open-Athena/MarinFold/issues/137), [#150](https://github.com/Open-Athena/MarinFold/issues/150)) and re-run accuracy on all of them — every number we quote is still from a model trained on 4M AFDB proteins.
* Eric: evals and analysis across the scaling ladder ([#154](https://github.com/Open-Athena/MarinFold/issues/154)) — scoring a checkpoint is ~4 minutes now — and babysit the augmentation experiment ([#166](https://github.com/Open-Athena/MarinFold/issues/166)).
* Zack: finish the 8×H100 [#156](https://github.com/Open-Athena/MarinFold/issues/156) comparison, summarize validation curves + telemetry, and decide whether repeats / LR sweeps are needed to get the noise under control; fix or bypass the GB200 multi-GPU eval issue.
* Allen on why we fall off with protein length ([#96](https://github.com/Open-Athena/MarinFold/issues/96)) — Jandom's idea of folding individual subdomains is a good first test.
* Jacob on a report of the multimer content of AFDB / ESM-Atlas ([#145](https://github.com/Open-Athena/MarinFold/issues/145)).
* Alex's bio2token baseline ([#133](https://github.com/Open-Athena/MarinFold/issues/133)) still needs a model trained on the merged neural-token documents.
* The pause-token dataset ([#124](https://github.com/Open-Athena/MarinFold/issues/124)) is unclaimed if anyone wants it — the data is [ready](https://huggingface.co/buckets/open-athena/MarinFold/tree/data/document_structures/contacts_v1_think).
