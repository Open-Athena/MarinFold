...
def test_levanter_loss_applies_sparse_weighted_targets_after_one_forward() -> None:
    ...
    np.testing.assert_allclose(
        np.asarray(batch.target_weights)[:4],
...
def test_levanter_materializes_block_causal_attention_with_segment_isolation() -> None:
    ...
    np.testing.assert_array_equal(
        materialized.rearrange((Pos, hax.Axis("key_position", 4))).array,
        np.asarray(
            (
...
def test_levanter_lowers_implicit_shifted_targets() -> None:
    ...
    assert batch.target_position_count == 2
    np.testing.assert_array_equal(np.asarray(batch.target_positions)[:2], (0, 1))
    np.testing.assert_array_equal(np.asarray(batch.target_ids)[:2], (4, 5))
    np.testing.assert_array_equal(np.asarray(batch.target_weights)[:2], (1.0, 1.0))
