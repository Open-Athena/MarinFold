...
class FixedQuotaShardBuilder:
    """Pickleable shard builder for multiprocessing fixed-quota datasets."""
    ...
    def __call__(self, epoch: int, shard_index: int) -> tuple[PackedDocuments | None, ...]:
        return construct_fixed_quota_shard(
...
def construct_fixed_quota_shard(
    *,
    data_prefix: str,
    ...
    """Read, documentize, pack, and slot one fixed-quota shard."""
    shard_name = shard_name_template.format(
        shard_index=shard_index,
        total_shards=total_shards,
    )
    shard_path = f"{data_prefix.rstrip('/')}/{shard_name}"
    ...
    row_rng = np.random.default_rng(np.random.SeedSequence([seed, epoch, shard_index, 0]))
    documents: list[Document] = []
    rows = table.to_pylist()
    for row_index in row_rng.permutation(table.num_rows):
        document = generate_document(rows[int(row_index)])
        if document is None:
            continue
    ...
    slot_rng = np.random.default_rng(np.random.SeedSequence([seed, epoch, shard_index, 1]))
...
class MPFixedQuotaShardDocumentDataset(AsyncDataset[Example], Generic[Example]):
    """Fixed-quota document dataset with multiprocess shard-slot prefetch.

    Worker processes build packed shard slots from parquet rows. The main
    process converts those slots into model examples so JAX arrays and device
    state are not created in subprocesses.
    """
    ...
    def __init__(
        ...
        *,
        ...
        builder = FixedQuotaShardBuilder(
            data_prefix=self.data_prefix,
            columns=self.columns,
            generate_document=self.generate_document,
            total_shards=self.total_shards,
        ...
        self._slot_dataset = MPQueueShardDocumentDataset[
            PackedDocuments | None
        ](
            build_shard=builder,
            num_shards=self.num_shards,
            examples_per_shard=self.examples_per_shard,
            seed=self.seed,
            transform_workers=self.transform_workers,
    ...
    async def async_len(self) -> int:
        raise ValueError("MPFixedQuotaShardDocumentDataset is an infinite stream")
