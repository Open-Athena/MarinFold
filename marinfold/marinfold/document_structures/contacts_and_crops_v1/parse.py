...
"""Structure analysis for the contacts-and-crops-v1 format.

Local to this document-structure package. Imported by ``generate.py`` and
``cli.py``.

Identical to contacts-and-coordinates-v1's (ccoord) parse layer — this
format reuses ccoord's parse path verbatim. It needs two things from a
structure that contacts-v1 already computes — the residue sequence and the
pyconfind side-chain contacts — plus one thing contacts-v1 throws away: the
per-atom 3D coordinates of every heavy atom. So :func:`analyze_coordinates`
runs contacts-v1's
...
class AnalyzedCoordStructure:
    """Everything the crop document builder needs from one structure.
