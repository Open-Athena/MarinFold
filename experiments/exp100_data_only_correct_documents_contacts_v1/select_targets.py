...
"""Stage A (local, CPU): sample exp100 only-correct targets from the contacts-v1
**train** split.

For the **validation** phase exp100 reuses exp98's exact ``data/targets.parquet``
(1000 round-0 proteins) so the only-correct documents are directly comparable to
exp98's free rollouts. For the **full** regeneration phase this script selects the
training proteins to regenerate (raise ``--n`` / drop the round-0 restriction to
cover the whole unique-protein set).
...
global_plddt, struct_cluster_id, round, shard``.
    ...
    uv run python select_targets.py --n 1000 --max-len 512 --min-contacts 5 \
        ...
        --gcs-out gs://marin-us-east5/protein-structure/MarinFold/exp100_only_correct_contacts_v1_train/targets.parquet
"""
