...
VERBATIM COPY of ``experiments/exp232_sweep_cv1_decontam/training_contract.py``.

It lives here because Iris bundles the *current working directory*, and this
project has to be submittable on its own — the repository root carries no
``pyproject.toml`` for the worker to sync. Importing across experiment
directories would mean bundling the whole repository, which exceeds the 25MB
bundle limit several times over.

Do not edit this file. ``tests/test_contract_copy.py`` asserts it is
byte-identical to exp232's, modulo this docstring, so the two cannot drift.
"""
