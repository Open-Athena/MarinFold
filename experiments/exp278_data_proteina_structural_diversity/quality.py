...
def ca_geometry(
    coordinates: np.ndarray, *, cis_proline_steps: np.ndarray | None = None
    ...
    invalid_steps = (adjacent < 3.4) | (adjacent > 4.2)
    if cis_proline_steps is not None:
        if cis_proline_steps.shape != adjacent.shape or cis_proline_steps.dtype != bool:
            raise ValueError("Expected one boolean cis-proline flag per peptide bond")
        allowed_cis = cis_proline_steps & (adjacent >= 2.7) & (adjacent <= 3.4)
        invalid_steps &= ~allowed_cis
    break_count = int(np.sum(invalid_steps))
    return {
...
def backbone_geometry(structure: gemmi.Structure) -> dict[str, float | int | bool]:
    """Distinguish validated cis-proline bonds from broken C-alpha traces.

    Cis-proline needs a short CA step, near-planar cis omega (within 20 degrees),
    and a connected C-N bond (1.15–1.60 A). Other short steps remain rejections.
    This explicitly repairs the trans-only CA-distance screen without allowing
    arbitrary compressed traces. Counts remain available for separate audits.
    """
    if len(structure) != 1 or len(structure[0]) != 1:
        raise ValueError("Geometry requires one model and one chain")
    residues = list(structure[0][0])
    coordinates = np.asarray([list(residue["CA"][0].pos) for residue in residues])
    cis_proline = np.zeros(len(residues) - 1, dtype=bool)
    for index, (first, second) in enumerate(
        zip(residues[:-1], residues[1:], strict=True)
    ):
        if second.name != "PRO":
            continue
        carbon, nitrogen = first["C"][0].pos, second["N"][0].pos
        omega = math.degrees(
            gemmi.calculate_dihedral(
                first["CA"][0].pos, carbon, nitrogen, second["CA"][0].pos
            )
        )
        cis_proline[index] = abs(omega) <= 20 and 1.15 <= carbon.dist(nitrogen) <= 1.60
    return {
        **ca_geometry(coordinates, cis_proline_steps=cis_proline),
        "cis_proline_bonds": int(cis_proline.sum()),
    }
