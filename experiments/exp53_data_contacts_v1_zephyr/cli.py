...
"""Stage B driver — generate contacts-v1 documents at scale on Iris/Zephyr.

This is the at-scale, Zephyr-parallelized runner. It consumes the Stage-A
selection manifest (``selection.py`` output: one row per selected
(entry, round) record, with ``gcs_uri`` + provenance) and, for each row,
fetches the structure and emits a contacts-v1 document by calling **straight
into** ``marinfold``'s generator (the per-row work lives in
``generate_rows.py``; exp53 re-implements no document logic — issue #53).

Run **once per split** so the published corpus keeps protein-docs's
``train`` / ``val`` / ``test`` layout, and because Stage A already numbered
each split's manifest shards in descending-round order, the per-shard output
inherits that order (round-4 first ... round-0 last). Examples in the README.
...
# Manifest columns we copy through (if present). ``entry_id`` + the cif
# source are required; the rest are provenance carried onto each output row.
...
def cmd_generate(args: argparse.Namespace) -> None:
    ...
    # One output file per input manifest shard preserves Stage A's
    # round-descending shard order; omit {shard} to collapse to a single file.
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python cli.py",
        description=f"{NAME} on zephyr — generate documents from a selection manifest.",
    )
    sub = parser.add_subparsers(dest="cmd", required=True)
    p = sub.add_parser("generate", help="Generate contacts-v1 documents at scale.")
    p.add_argument("--input", type=str, required=True,
                   help="Selection-manifest parquet glob (one split), e.g. "
                        "gs://.../selection_manifest/train/shard_*.parquet.")
    ...
    p.add_argument("--out", type=str, required=True,
                   help="Output path (.parquet/.jsonl). Include a {shard} "
                        "placeholder (e.g. contacts_v1-{shard:05d}-of-{total:05d}"
                        ".parquet) to write one file per input shard.")

    # Zephyr worker resources (exp5 lesson: 1 CPU/worker, scale via max-workers).
