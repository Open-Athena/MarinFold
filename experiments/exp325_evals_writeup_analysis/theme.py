...
TITLES = {
    ...
    "02c_accuracy_confidence": "Does confidence track structural accuracy?",
...
CAPTIONS = {
    "02c_accuracy_confidence": "Original ESMFold2 structure accuracy versus Helico confidence after conditioning on its extracted contact map. Colors identify the five natural proteins with MSA depth <10; each has 100 seeded ESMFold2 predictions. Diamonds mark oracle contacts. In the source-structure view, oracle accuracy is 1 by definition because its source is the experimental reference. The menu can isolate each protein and also shows the measured accuracy of the resulting Helico structures, including the oracle-conditioned ones; GDT-TS and lDDT are separate options. Confidence always comes from the highest-ranking of three Helico samples per map. No seed is filtered by accuracy or confidence, and the 500 predictions represent only five proteins. The confidence axis has an explicitly marked break; the clash-penalized point at −99.41 is retained, and hover reports the unmodified score.",
    "02b_confidence": "All five natural FoldBench proteins with MSA depth <10 (test split). Each dot is one of 100 single-sequence ESMFold2 predictions, converted to a full contact/non-contact map and folded by Helico; diamonds use the ground-truth oracle map. Every map has the same eligible-pair mask and three Helico diffusion samples, selected by ranking_score. Contact counts may vary. Rank 1 is highest confidence among 101 maps; ties are shown as rank intervals. Vertical spread separates dots and has no numerical meaning. The menu switches to pLDDT of the same selected structures. These 500 decoys represent five proteins, not 500 independent biological examples. The ranking-score axis has an explicitly marked break to retain one clash-penalized score (−99.41) without compressing the other points; hover reports the unmodified score.",
