...
Adapted from exp277 evals/2026-09-13_rollout_v2 at dc3a1969. Adds raw
completion archives; the Modal launcher uses chunk=1 so each timing measures
one protein. Heavy optional GPU imports stay inside the worker.
...
retains the predictor timing/provenance required by repository policy. An explicit
flag can exclude nonterminating samples from voting while preserving their full
diagnostics and requested/usable rollout counts.
"""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--runner-tag", default="modal")
    ...
    for offset in range(0, len(pending), arguments.chunk):
        ...
        for item in per_record:
            ...
            for output, position_map in zip(record_outputs, position_maps, strict=True):
                archive.append({"text": output.outputs[0].text, "position_map": position_map,
                                "finish_reason": output.outputs[0].finish_reason,
                                "generated_tokens": len(output.outputs[0].token_ids)})
                if output.outputs[0].finish_reason != "stop":
                    continue
            ...
            write_json({"dataset": record["dataset"], "stem": record["stem"], "L": length,
                        "rollouts": archive},
                       f"{output_directory}/rollouts/{record['stem']}.json")
            ...
            timing_rows.append(
                {
                    ...
                    "runner_tag": arguments.runner_tag,
                    **hardware,
        ...
        for timing in timing_rows:
            timing["total_seconds"] = model_stage_seconds + model_load_seconds + time.monotonic() - group_started
        write_parquet(
