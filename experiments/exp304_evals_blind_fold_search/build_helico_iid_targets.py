...
def main() -> None:
    ...
    for pair_id in sorted(expected):
        ...
        if structure_meta["sequence_identity"] != 1.0:
            raise ValueError(f"{pair_id}: Fold1 observed sequence differs from union sequence")
        input_sequence = "".join(
            sequence[position]
            for position, _ in sorted(positions.items(), key=lambda item: item[1])
        )
        ...
        for record in frame.to_dict("records"):
            ...
            mapped = mapped_contacts(contacts, positions)
            rows.append({
                ...
                "n_contacts_mapped": len(mapped),
                "n_contacts_helico": sum(j - i >= 6 for i, j in mapped),
                "contacts": mapped,
        ...
        for kind, rollout, pairs in (
            ...
            mapped = mapped_contacts(pairs, positions)
            rows.append({
                "target_id": f"{pair_id}__{kind}", "pair_id": pair_id,
                "kind": kind, "rollout": rollout, "candidate_id": kind,
                "L": int(target.L), "sequence": input_sequence,
                "marinfold_finished": True,
    ...
    protocol = {
        ...
        "n_targets": len(rows),
        "input_sequence": "Fold1 observed chain sequence",
        "reference_coordinates_used_as_model_features": False,
        ...
        "helico_modal_app_urls": [
            "https://modal.com/apps/open-athena/main/ap-X8qQ3OhGqEMB51CGItDied",
            "https://modal.com/apps/open-athena/main/ap-1DKrtJR4Ips7WQLrX8iZv4",
        ],
        "run_started_utc": "2026-09-22T19:15:00Z",
