...
## Interpretation
The requested single full-corpus model is ready for evaluation. Language-model validation improved throughout most of the epoch, with a small increase from the best value at the final validation. This alone does not answer the redesign question: compare contact eval-val R-precision against the native-only winner while reporting the different training exposure. Contact-prediction quality has not yet been evaluated.
