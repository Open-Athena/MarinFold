...
"""Run the exp277 rollout-v2 evaluation entirely inside CoreWeave.

The CPU driver builds the exact 670-unit union required here: the current
eval-val and eval-denovo sets plus the legacy 554 comparison universe. It does
not deduplicate overlapping PDB stems because evaluation identity is the
``(dataset, stem)`` pair.
"""
...
def _build_evaluation_union(
    *,
    ...
    expected = {
        ...
        "overlapping_stems_retained": 112,
    }
    ...
    *,
    ...
    for checkpoint in checkpoints:
        ...
        for shard_idx in shards:
            name = f"e277-{run_id}-{phase[:2]}-{checkpoint.job_label}-s{shard_idx:02d}"
...
def main() -> None:
    ...
    parser.add_argument(
        "--suite", choices=sorted(CHECKPOINT_SUITES), default="exp277"
    )
    ...
    targets_uri = f"{root}/inputs/eval_targets_670.parquet"
    ground_truth_uri = f"{root}/inputs/gt_universe_670.jsonl"
