...
def aggregate_subsets(
    ...
    *,
    ...
    """Aggregate the union over legacy, eval-val, and eval-denovo cuts."""

    subsets = {"universe_670": set(ordered_units)}
    ...
    if not all(units <= subsets["universe_670"] for units in subsets.values()):
...
def finalize(
    *,
    ...
    scratch = Path("/tmp/exp277_rollout_v2_finalize")
    ...
    manifest = {
        ...
        "checkpoint_source": {
            ...
            "transport": (
                "pre-existing exp277 CoreWeave HF directory verified in place; "
                "no model weights moved"
            ),
