...
Identical to the v1 parser (v2 is a serialization-only delta — it
sees exactly the same structures, the same atoms, and the same
pLDDT values). Lifted into this experiment dir so the experiment
stays self-contained for ``generate.py`` and the tests; the per-
residue parse machinery is small enough that a copy is cheaper than
a path import + relative-import gymnastics.

Imports ``AMINO_ACIDS`` / ``ATOM_NAMES`` from this experiment's
``vocab.py``, which re-exports them from the marinfold v1 vocab —
so the two parsers can never disagree on which atoms are in-vocab.
"""
...
class ParsedStructure:
    """One parsed polymer chain, ready for doc generation."""
...
def _vocab_safe_atoms(gemmi_residue) -> tuple[tuple[str, float, float, float], ...]:
    """Heavy atoms whose names are present in the shared atom vocab."""
...
# --------------------------------------------------------------------------
# Atom-position helpers used by generation
# --------------------------------------------------------------------------
