...
thin stratum never silently shrinks the pilot below the requested size.
    ...
    uv run python pilot.py --selected /data/exp294/selection/selected.parquet \\
        --out /data/exp294/pilot --size 50000
"""
    ...
    con.execute(
        ...
        SELECT *,
            row_number() OVER (
                ...
                -- Hash of the id, not random(): reproducible from the manifest
                -- alone, and independent of row order.
                ORDER BY hash(model_id), model_id
            ) AS stratum_rank,
            ...
            row_number() OVER (
                PARTITION BY stratum, tax_id_a ORDER BY hash(model_id), model_id
            ) AS taxon_rank
        FROM strata
        """
    )
...
def build(selected: Path, out_dir: Path, *, size: int = DEFAULT_PILOT_SIZE) -> dict[str, Any]:
    """Write the pilot manifest and its stratum accounting."""
    ...
    con.execute(
        f"""
        COPY (SELECT * FROM pilot ORDER BY source_tar_uri, model_id)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps(build(args.selected, args.out, size=args.size), indent=2))
    return 0
