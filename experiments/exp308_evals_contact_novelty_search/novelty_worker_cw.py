#!/usr/bin/env python
"""Generate contact-block rollouts with previous-rollout novelty penalties."""
...
from novelty_policy import effective_epsilon, pair_candidates, sample_candidate
...
def decode(llm: Any, states: list[Rollout], stem: str, beam_width: int,
           ...
           previous_counts: Counter[tuple[int, int]], epsilon: float,
           decay_contacts: int, max_statements: int | None = None) -> None:
    """Search complete position pairs using counts from previous rollout waves."""
        ...
        if contacts:
            ...
            for state, result in zip(contacts, beams):
                ...
                if not candidates:
                    ...
                    state.malformed += 1
                    continue
                current_epsilon = effective_epsilon(
                    epsilon, len(state.beam_choices), decay_contacts
                )
                selected, adjusted, prior_count = sample_candidate(
                    candidates, state.rng, previous_counts, current_epsilon
                )
...
def main() -> None:
    ...
    parser.add_argument("--decay-contacts", type=int, required=True,
                        help="zero means a constant penalty")
    ...
    effective_epsilon(args.epsilon, 0, args.decay_contacts)
    ...
    if args.selection == "pilot":
        targets = [row for row in targets if row["pilot"]]
    elif args.selection == "test":
        targets = [row for row in targets if row["split"] == "test" and row["primary"]]
    ...
    if not mine:
        print(f"[exp308] no targets for shard {shard}/{n_shards}", flush=True)
        return
    print(f"[exp308] shard {shard}/{n_shards}: {len(mine)} {args.selection} targets", flush=True)
    ...
    llm = LLM(model=args.model, dtype="bfloat16", max_model_len=CONTEXT,
              ...
              generation_config="vllm", max_num_seqs=256,
              max_logprobs=2 * args.beam_width, seed=308)
    model_load = time.perf_counter() - load_start
    gpu = torch.cuda.get_device_properties(0)
    mode = (f"b{args.beam_width}_e{str(args.epsilon).replace('.', 'p')}"
            f"_d{args.decay_contacts}_w{args.wave_size}")
    meta = {
    ...
    for position, target in enumerate(mine, 1):
        stem, sequence = target["stem"], target["sequence"]
        base = f"{args.out.rstrip('/')}/{mode}/foldswitch/{stem}"
        ...
        if filesystem.exists(raw_path) and filesystem.exists(fsspec.core.url_to_fs(timing_uri)[1]):
            print(f"[exp308] skip {stem}", flush=True)
            continue
        ...
        previous_counts: Counter[tuple[int, int]] = Counter()
        for first in range(0, len(states), args.wave_size):
            wave = states[first:first + args.wave_size]
            decode(llm, wave, stem, args.beam_width, int(contact_id), int(end_id),
                   previous_counts, args.epsilon, args.decay_contacts,
                   args.max_statements)
            for state in wave:
                if state.finished:
                    previous_counts.update(state.contacts)
        elapsed = time.perf_counter() - start
        rows = [{
            "cohort": "foldswitch", "dataset": target["dataset"], "stem": stem,
            ...
            "wave": state.index // args.wave_size,
            "epsilon_initial": args.epsilon,
            "decay_contacts": args.decay_contacts,
        ...
        print(f"[exp308] {position}/{len(mine)} {stem} L={len(sequence)} "
