#!/usr/bin/env python
"""Seal novelty-search shortlists, then score both reference contact modes."""
...
EXP306 = EXPERIMENTS / "exp306_evals_contact_block_beam_search"
...
def raw_files(mode: str, selection: str) -> list[Path]:
    """Return exactly the preselected pilot or primary test raw tables."""
    targets = pd.read_csv(HERE / "data" / "targets.csv")
    if selection == "pilot":
        expected = set(targets.loc[targets.pilot, "stem"])
    elif selection == "test":
        expected = set(targets.loc[(targets.split == "test") & targets.primary, "stem"])
    elif selection == "all":
        expected = set(targets.stem)
    else:
        raise ValueError(selection)
    root = HERE / "_cache" / mode / "foldswitch"
    files = sorted(root / f"{stem}.parquet" for stem in expected)
    missing = [path.name for path in files if not path.exists()]
    if missing:
        raise FileNotFoundError(f"{len(missing)} missing {selection} results: {missing[:5]}")
    return files


def seal(files: list[Path], mode: str, selection: str, n_rollouts: int) -> None:
    ...
    output = HERE / "data" / f"sealed_{selection}_{mode}.csv"
...
def score(files: list[Path], mode: str, selection: str, n_rollouts: int) -> None:
    """Reveal references only after verifying the frozen shortlist hash."""
    shortlist_path = HERE / "data" / f"sealed_{selection}_{mode}.csv"
    ...
    beam4 = pd.read_csv(EXP306 / "data" / "foldswitch_beam4.csv").set_index("pair_id")
    rows = []
    for path in files:
        frame = pd.read_parquet(path).sort_values("rollout")
        pid = path.stem
        ...
        rows.append({
            ...
            "beam4_seconds": beam4_seconds,
            "time_ratio_vs_beam4": beam_seconds / beam4_seconds,
            "mean_prior_count_first20": float(pd.Series(
            ...
            "beam4_dual_pool": bool(beam4.loc[pid, "dual_pool"]),
            "beam4_dual_blind": bool(beam4.loc[pid, "dual_blind"]),
        })
    per = pd.DataFrame(rows).sort_values("pair_id")
    out = HERE / "data"
    per.to_csv(out / f"foldswitch_{selection}_{mode}.csv", index=False)
    cohorts = {
        "selection": pd.Series(True, index=per.index),
        "primary": per.primary,
    }
    summary = []
    for name, selected in cohorts.items():
        ...
        summary.append({
            ...
            "beam4_dual_pool": int(group.beam4_dual_pool.sum()),
            "beam4_dual_blind": int(group.beam4_dual_blind.sum()),
            ...
            "total_time_ratio_vs_beam4": float(group.beam_seconds.sum()
    ...
    report.to_csv(out / f"foldswitch_summary_{selection}_{mode}.csv", index=False)
...
def main() -> None:
    ...
    files = raw_files(args.mode, args.selection)
    if args.phase == "seal":
        seal(files, args.mode, args.selection, args.n_rollouts)
    else:
        score(files, args.mode, args.selection, args.n_rollouts)
