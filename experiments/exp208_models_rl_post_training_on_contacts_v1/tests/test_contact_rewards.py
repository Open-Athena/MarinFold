...
"""Unit tests for exp208's dense per-contact reward.

The parity test against exp163's ``rollout_metrics.score_rollout`` is the
important one: exp208's document-level return has to be the SAME number the
...
def test_section_f1_matches_exp163_scorer(sections):
    """exp208's per-section F1 must equal exp163's published metric, exactly."""
