...
```
--n-rollouts 100 --temperature 1.0 --top-p 1.0 --top-k -1 --contact-mult 6 --seed 0
```
...
### Note on `--top-p 1.0` — deliberate, and it changes comparability

Every published MarinFold rollout number, including both disputed values below,
was produced at **`top_p = 0.95`**. This request deliberately uses **1.0**, i.e.
no nucleus truncation at all. Two things follow:

- Your number will **not** be directly comparable to 0.5873 / 0.6103. Treat it as
  measuring this checkpoint under a slightly different (and simpler) sampling
  rule, not as adjudicating those two.
- **If settling that disagreement is also a goal, run both**: the same command
  with `--top-p 0.95` into a separate output prefix. The marginal cost is one
  extra pass and it is the only version that speaks to the disputed numbers.

Why 0.95 was never obviously right: the only sweep behind it (exp82,
`data/results_rollout_sweep_dev.txt`) covered **16 dev proteins** and read, in
long-band R-precision — pairwise reference 0.160; T1.0/p0.95 **0.189**;
T0.7/p0.95 **0.192**; T0.7/p1.0 **0.191**; T1.0/p1.0 **0.184**. The p=0.95 and
p=1.0 rows are within noise of each other on 16 proteins, and that sweep ran
`top_k 50`, which the settled recipe later dropped to off (#142 traced
under-generation to a finite top_k) — so no row in it is the recipe in use today.
0.95 is a convention that keeps published numbers comparable, not a tuned
optimum. The one robust signal there is rollout >> pairwise.
...
(all-range) — **both at `top_p = 0.95`**. The gap is ~10x the reproducibility span
of this recipe (four evaluations of one unchanged checkpoint span 0.0023).

At `top_p = 1.0` we have **no prior** to compare against, so there is no expected
value; report what you get. If you also run the `top_p = 0.95` arm, that one
should land clearly on one side of 0.5873 / 0.6103 and is the one that settles
the disagreement.
