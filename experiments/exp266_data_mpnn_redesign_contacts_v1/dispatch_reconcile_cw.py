...
"""Dispatch the ESM-arm id-set reconciliation to CoreWeave.
...
DOCUMENTS_GLOB = os.environ.get("EXP266_RECONCILE_DOCS",
                                f"{S3_PREFIX}/esm_documents/*.parquet")
JOB_NAME = os.environ.get("EXP266_RECONCILE_JOB", "exp266-reconcile")

WORK_DIR = "/tmp/exp266"
WORKER_FILES = ("reconcile_esm.py", "esm_shard_map.json.gz")
...
def build_bootstrap(*, workers: int) -> str:
    return f"""
set -euo pipefail
echo "[exp266-reconcile] host=$(hostname) image={IMAGE}"

{FSSPEC_VIRTUAL_ADDRESSING_EXPORT}
echo "[exp266-reconcile] HF_TOKEN=${{HF_TOKEN:+present}} FSSPEC_S3=${{FSSPEC_S3:+present}}"
...
$PY -m pip install --quiet "huggingface_hub>=1.5" fsspec s3fs boto3 pyarrow numpy

export PYTHONPATH={WORK_DIR}:${{PYTHONPATH:-}}
exec $PY {WORK_DIR}/reconcile_esm.py \\
...
def main() -> None:
    ...
    if args.dry_run:
        print(f"[exp266-reconcile] DRY RUN over {DOCUMENTS_GLOB}")
