...
"""Dispatch the ESM-arm corpus verification to CoreWeave.
...
def build_bootstrap(*, workers: int) -> str:
    return f"""
set -euo pipefail
echo "[exp266-verify] host=$(hostname) image={IMAGE}"

{FSSPEC_VIRTUAL_ADDRESSING_EXPORT}
echo "[exp266-verify] HF_TOKEN=${{HF_TOKEN:+present}} FSSPEC_S3=${{FSSPEC_S3:+present}}"
...
exec $PY {WORK_DIR}/verify_corpus.py \\
    --documents-glob "{DOCUMENTS_GLOB}" \\
    --expected-shards 3338 \\
    --expected-backbones {EXPECTED_BACKBONES} \\
    --designs {DESIGNS} \\
    --deep-shards 6 \\
    --exact-tokens
""".strip()
...
def main() -> None:
    ...
    if args.dry_run:
        print(f"[exp266-verify] DRY RUN over {DOCUMENTS_GLOB}")
