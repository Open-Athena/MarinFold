...
| 4 | 2026-08-31 | #232 `m2-p06` **training** checkpoint (step 363,000), the same checkpoint as read 3 | [#250](https://github.com/Open-Athena/MarinFold/issues/250)'s figures need per-protein contact scores over the same 333 monomers the Helico arms are folded on, and PR #257 left eval-test unscored for this checkpoint. Scored independently of read 3 — a separate 8xA100 run under exp82's recipe, launched before read 3's numbers were known. One checkpoint, fixed before the run; nothing was selected on the result. | **R-precision (all)** eval-test **0.5681**, eval-val 0.5557, eval-denovo 0.6123. Read 3's CoreWeave run of the same checkpoint gives 0.5693 / 0.5561 / 0.6110 — the two independent pipelines agree within 0.0013. |
| 5 | 2026-09-08 | exp157 fixed/RoPE-only arm, learned RoPE delta, and learned RoPE delta + `1e-3` L2 prior at step 71,359 | Final writeup read for #157 after the three position-embedding arm identities were fixed by training loss and eval-val was already the appropriate iteration set. This did **not** include the true vanilla no-RoPE learned-token baseline (`contacts-v1-exp117-1.5B`), which remains pending on exp245. | **R-precision (all)**: fixed/RoPE-only 0.362, RoPE-delta **0.410**, RoPE-delta+L2 0.182. Same-manifest context, not matched recipe controls: #232 m2-p06 decontam 0.538, #232 m1-p02 decontam 0.493, #199 cooldown 0.613, Protenix-v2 single-seq 0.265, ESMFold 0.753, ESMFold2 0.792, Protenix-v2 + MSA 0.845, seq-KNN 0.582 (unfiltered corpus) / 0.426 (decontaminated). |
| 6 | 2026-09-09 | Eric's vanilla no-RoPE learned-position-token baseline `contacts-v1-exp117-1.5B` / `prot-exp117-cv1-s02-1_5b-e16-lr3p162e-3-wd0p2-bs256-europe-west4` at step 35,679 | Follow-up final-writeup read for #157 after realizing row 5 omitted the actual matched vanilla training-recipe control. The model identity was fixed historically by exp117 training/validation loss; this read was only to put that control onto the same exp245 manifest as the exp157 RoPE arms. | **R-precision (all)**: eval-val 0.395, eval-test 0.384, eval-denovo 0.534. Compared with row 5, RoPE-delta is +0.018 on eval-val and +0.026 on eval-test, while vanilla is +0.009 on eval-denovo. |
