...
"""Train the 1.4B Llama protein model with a loss mask that focuses training
on distance-bin predictions only.

Same data, tokenizer, model, and hyperparameters as ``train_protein_1b.py`` —
the only change is the loss weight. A distance statement has 6 tokens::
...
Usage::
    ...
    python train_protein_1b_distance_masked.py
"""
...
from protein_train_common import (
    PROTEIN_TOKENIZER,
...
# See protein_distogram_eval.py for how to add targets. The eval components
# use `PrebuiltLmDatasetFormat` with a pre-computed loss_weights column in
# the parquet, so the distance-mask loss_weight_fn (applied to the train +
# val components) does NOT apply to them — they see only the final GT
# distance-bin position as their training target, which is what we want.
PROTEIN_MPNN_REDESIGNS_SOURCE = "gs://marin-us-east5/protein-structure/protein-mpnn-redesigns/v1/redesigns.jsonl"
# 100 pairs per (target, N) keeps a full eval pass fast. The default of 1000
# made eval take ~78 min on 132 datasets, longer than most training
# intervals. Drop to 100 and the full eval runs in ~8 min — acceptable
# overhead at steps_per_eval=1000 below.
...
# -- Training config --
#
# LR bumped 3x from the prior 3.5e-4 unmasked run to 1.05e-3. The distance-
# masked loss reduces the effective per-batch gradient signal (only ~1/6 of
# distance-statement tokens contribute), so a bigger learning rate is
# warranted. Batch, warmup, and step count kept the same as train_protein_1b.py
# for comparability.
