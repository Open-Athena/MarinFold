...
Targets the run produced by ``train_protein_100m_unmasked.py`` (loss applied
to every position, no distance-bin-only mask). ``CHECKPOINT_STEP`` is used
only for the output directory name (``hf/step-{CHECKPOINT_STEP}``);
...
Usage::
    ...
    uv run iris --config=lib/iris/examples/marin.yaml job run \\
        ...
        -- python -m export_protein_100m_unmasked
"""

from marin.execution.executor import executor_main

from protein_train_common import build_hf_export_step
from train_protein_100m_unmasked import (
    protein_llama_100m,
