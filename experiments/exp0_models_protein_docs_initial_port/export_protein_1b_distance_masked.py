...
(wandb run `protein-contacts-1b-3.5e-4-distance-masked-7d355e`). Writes the
HF checkpoint to ``{TRAINING_OUTPUT}/hf/step-{CHECKPOINT_STEP}`` so multiple
...
Usage::
    ...
    uv run iris --config=lib/iris/examples/marin.yaml job run \\
        ...
        -- python -m export_protein_1b_distance_masked
"""
...
from train_protein_1b_distance_masked import (
    PROTEIN_TOKENIZER,
    protein_llama_1b,
...
# Permanent checkpoints in this run lack metadata.json (only the latest temp
# does), so we point at the parent dir and let levanter pick whatever loadable
# ckpt is current at job start. CHECKPOINT_STEP is used only for the output
# directory name.
