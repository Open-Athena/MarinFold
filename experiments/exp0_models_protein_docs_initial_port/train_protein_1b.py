...
This trains a model from scratch on a custom protein structure vocabulary
(~2840 tokens) encoding amino acid sequences, inter-residue contacts, and
atomic distances.
...
# Use pack=True to avoid concat-and-split, which would create partial
# documents. Protein docs are nonsensical if you only see the latter half
# without the header.
...
# See protein_distogram_eval.py for how to add targets.
#
# `redesigns_source` points at the JSONL produced by an MPNN-style
# redesign script. If the file doesn't exist yet (redesign script not run),
# the benchmark silently skips redesigned targets, so it's safe to
