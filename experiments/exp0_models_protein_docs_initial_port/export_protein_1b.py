...
The training run from ``train_protein_1b.py`` writes a Levanter-native
(tensorstore/ocdbt) checkpoint at
``gs://marin-us-east5/checkpoints/protein-contacts-1b-2.5e-4-780930/checkpoints/step-22477``
but no HF export. vLLM needs an HF checkpoint to load, so this step
materializes one at ``.../hf``.
...
protein_model_1b_hf = convert_checkpoint_to_hf_step(
    ...
    use_cpu=True,
    # Pin output to the path `eval_protein_contacts.py` expects, matching
    # the original training run's bucket layout.
