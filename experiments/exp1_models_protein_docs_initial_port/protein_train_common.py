...
and any additional eval components on top of these shared bits.
"""
...
from create_protein_tokenizer import create_protein_tokenizer
...
def build_hf_export_step(
    *,
    ...
    to a literal gs:// path to bypass that dependency — useful when you want
    to snapshot a checkpoint while training is still in progress.
    ...
    Args:
        ...
        checkpoint_path_override: When set, use this literal gs:// path instead
            of the dependency-creating ``output_path_of(train_step, ...)``.
            The export step then has no marin-executor dependency on
            ``train_step`` and will run immediately, regardless of
            ``train_step``'s status.
    """
