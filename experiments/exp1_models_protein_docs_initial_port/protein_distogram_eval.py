...
experiment. Marin's executor auto-builds the 6 eval-data parquets for
your new target on the next run and caches them thereafter.
...
class ProteinTarget:
    ...
    # the prompt uses these 3-letter residue names instead of the PDB's
    # native sequence. Ground-truth CA coordinates still come from the PDB
    # structure, so the eval asks "given this (redesigned) sequence, what's
    # the CA-CA distance in the native-like structure?". Length must match
    # the parsed chain length; all entries must be standard AAs.
...
# from the deposited label — so we fetch the .pdb1 biological assembly and
# use the first chain encountered.
#
...
# benchmark in general. We substitute the next valid entry (7ur7, 63
# residues, CSV #11) to keep a set of 10 monomer targets.
...
def _fetch_pdb_text(pdb_id: str, assembly: int = 0) -> str:
    """Fetch the PDB file from RCSB. ``assembly=0`` is the deposited asym
    unit; 1+ are biological assemblies."""
    pdb_lc = pdb_id.lower()
    pdb_uc = pdb_id.upper()
    if assembly == 0:
        urls = (
...
class BuildDistogramEvalDataConfig:
    ...
    # pack=True for PrebuiltLmDatasetFormat). Keeps things simple at the cost
    # of 100 eval examples per target times 8192 tokens of mostly-pad
    # (~3 MB per parquet).
...
def build_distogram_eval_data(config: BuildDistogramEvalDataConfig) -> None:
    ...
    for pair in pairs:
        ...
        # is `loss_weight[i]` weights the loss of predicting `tokens[i+1]`,
        # so to score the <d_value> prediction (and <d_value> is the last
        # real token at index N-1) we set loss_weight[N-2] = 1 — not N-1,
        # which would be silently zeroed by the causal mask (predicts
        # tokens[N], out of bounds). Padding positions get weight 0 and have
        # no effect on the loss. Causal attention means pad positions see
        # the prompt but the prompt never sees the pads — no leakage.
        if real.shape[0] < 2:
    ...
    # Pre-build the levanter tree-cache so training doesn't have to. During
    # the last run, training-time cache builds were spawning sequential
    # zephyr sub-pipelines (~37 s each, 72 datasets, 45 min total) that got
    # killed every time the preemptible TPU worker was reclaimed, and the
    # job never made it past the cache-build phase in 17 hours. By building
    # the cache inside this (CPU, non-preemptible) ExecutorStep, the result
    # persists at the step's versioned output_path and any training run
    # just `TreeCache.load()`s it.
    from levanter.data.sharded_datasource import UrlDataSource
    from levanter.data.text.cache import build_lm_dataset_cache

    # Matches the path that training will look for:
    # LmDataConfig.build_caches() appends the split name to the component's
    # cache_dir. Our DatasetComponent sets cache_dir = step/"cache", so the
    # validation cache lives at {output_path}/cache/validation.
    ...
    build_lm_dataset_cache(
        ...
        tokenizer=tokenizer,
        # Pre-tokenized examples: no BOS/EOS enforcement needed.
        enforce_eos=False,
    )
...
def build_distogram_eval_step(
    ...
    *,
    ...
    return ExecutorStep(
        ...
        # versioned() is required for scalar fields to factor into the
        # step's output-path hash. Plain dataclass fields are IGNORED by
        # marin's hasher (only VersionedValue + InputName contribute), so
        # without these wraps changes to n_pairs / n_prompt_contacts /
        # max_seq_len / etc. would silently reuse stale caches under the
        # same output-path suffix.
...
def load_redesigned_targets(
    ...
    *,
    ...
    Args:
        source_jsonl: Path (local or gs://) to the JSONL file. If it
            doesn't exist, returns [] and logs an info message (so the
            training config doesn't break before redesigns have been
            generated).
        method_filter: If set, only records with this method are used.
        base_targets: Ignored; kept for API symmetry with
            ``distogram_eval_benchmark``.
    """
    import json as _json

    del base_targets  # unused, see note above
    # Config-build must succeed even if (a) the redesigns file hasn't been
    # generated yet, or (b) local GCS credentials are temporarily bad —
    # neither should block the training experiment from loading.
    try:
...
def distogram_eval_benchmark(
    ...
    *,
    ...
    Usage in a training experiment::
        ...
        from protein_distogram_eval import distogram_eval_benchmark

        bench = distogram_eval_benchmark(
            PROTEIN_TOKENIZER,
            redesigns_source="gs://marin-us-east5/.../redesigns.jsonl",
        )
    ...
    Args:
        ...
        redesigns_source: optional JSONL path produced by an MPNN-style
            redesign script. When set, one extra ``ProteinTarget`` per
            redesign record is added to the benchmark. Missing path is
            silently skipped.
    """
    ...
    for target in targets:
        for n in n_values:
            ...
            # `step / "..."` creates an InputName that marin's executor
            # resolves to the concrete GCS path once the step has an
            # output_path. We put the levanter tree-cache in a sibling
            # subdir next to the parquet so each (target, N) gets its
            # own cache and there's no sharing.
