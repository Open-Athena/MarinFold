...
ARMS = ("cluster_k3", "random_k3", "cluster_k5", "random_k5")
...
def score_pool(
    ...
    fold1_hits = [
        bool(item["fold1_hit"]) and bool(ok) for item, ok in zip(scored, valid)
    ]
    fold2_hits = [
        bool(item["fold2_hit"]) and bool(ok) for item, ok in zip(scored, valid)
    ]
    oracle_fold1 = any(fold1_hits)
    oracle_fold2 = any(fold2_hits)
    return {
...
def main() -> None:
    ...
    for target in targets.sort_values(["L", "stem"]).itertuples():
        iid = pd.read_parquet(
            args.root / "iid100" / "foldswitch" / f"{target.stem}.parquet"
        ).sort_values("rollout")
        ...
        for arm in arms:
            branch = pd.read_parquet(
                args.root / arm / "foldswitch" / f"{target.stem}.parquet"
            ).sort_values("rollout")
            ...
            for variant, include_seed in (("continuation", False), ("complete", True)):
                maps = iid_maps[:50] + branch_maps(branch, include_seed)
                rows.append(
                    {
                        "arm": arm,
                        "map_variant": variant,
                        "pair_id": target.stem,
                        **score_pool(maps, valid, truth[target.stem]),
                    }
                )
