...
"""Run the exp232 rollout-v2 evaluation entirely inside CoreWeave.

The CPU driver mirrors only the immutable public evaluation inputs into the shared
CoreWeave S3 bucket, verifies all checkpoints in place, launches one smoke job per
checkpoint, launches the full 12-way H100 fanout, waits, and finalizes metrics.
"""
...
def _build_eval2_targets(
    *, legacy_uri: str, manifest_uri: str, destination_uri: str
    ...
    added = manifest[
        (manifest["dataset"] == "foldbench_rest") & (manifest["has_ground_truth"] == 1)
    ][["dataset", "stem", "length", "input_seq"]].rename(columns={"length": "L"})
    ...
    *,
    ...
    for model_slot, checkpoint in enumerate(checkpoints):
        ...
        for shard_idx in shards:
            name = (
                f"e232v2-{run_id}-{phase[:2]}-{checkpoint.job_label}-s{shard_idx:02d}"
            )
...
def main() -> None:
    ...
    parser.add_argument("--suite", choices=sorted(CHECKPOINT_SUITES), default="exp232")
