...
The #75 E8 checkpoint passed all four published legacy gates at tolerance 0.005; the largest absolute difference was 0.002553 (long-range R-precision). `build_comparison_data.py` reuses the prior #146, #166, exp232 sweep, #199 m1-p06 augmentation/cooldown, and Protenix-v2 results and adds only the recomputed #75 slice and the new training slice. `plot_primary_comparison.py` renders `plots/final_checkpoint_rprecision.png`, sorts boxes by mean all-range R-precision, and jointly fits historical and #232 decontaminated sigmoids with a shared upper asymptote and width plus one learned horizontal loss shift.
