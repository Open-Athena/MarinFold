...
`VARIANTS` mapping defines the opaque logical trials. `lr050` and `lr005` were
stopped by the operator on 2026-08-23 for overfitting; `lr010` and `lr100` remain
active. The entry point owns all training, schedule, data, validation,
...
- Regional replicas: three simultaneous regions per active logical trial.
...
- Maintain three distinct regional runs per unfinished trial. After one reaches
  `run_progress >= 1` and its expected checkpoint is reachable, mark it the winner
  and stop its nonterminal siblings.
...
The completed ingress read the original CoreWeave S3 objects independently into
each regional GCS bucket, then compared pinned object counts, byte totals, and
complete relative-path inventories. No region was populated from another GCS
region.
...
- 2026-08-23 11:35--11:47 UTC. Operator judged `lr050` and `lr005` to be
  overfitting and redirected the sweep. Both trials were stopped, `lr010` was
  restarted and raised to three regional replicas, and a new trial `lr100` was
  added at three regional replicas. `lr100` is `Variant("lr100", 1.00)` in the
  entry point, which holds the source peak LR instead of decaying to a lower
  target and leaves the final cooldown unchanged; no other training semantics
  changed. Operator Choices moved from two to three regional replicas per active
  trial, and the replica-count wording in Operating Policy follows. Effect: six
  active dispatches across `lr010` in `us-east5`, `us-west4` and `us-east1`, and
  `lr100` in `us-east1`, `europe-west4` and `us-east5`.
- 2026-08-23 13:14 UTC. Operator terminated the sweep. All six remaining
  dispatches were stopped and verified `killed` in Iris, every trial and
  regional run is marked stopped in SQLite, and the hourly monitoring schedule
  was cancelled. No trial reached `run_progress >= 1`, so no winner was declared
  and no checkpoint verification was performed. Effect: the sweep is closed; the
  regional caches, seed checkpoints and per-run checkpoints remain in place in
  each region's own bucket.
