"""Launch the one-pass Atlas row locator in AWS us-west-2."""
...
PLAN_PREFIX = "exp292/production-v1/esm/plan-v5/plan"
OUTPUT_PREFIX = "exp292/production-v1/esm/locator"


def main() -> None:
    """Stage the locator source and optionally start its bounded EC2 worker."""
    ...
    with tarfile.open(bundle, "w:gz") as archive:
        for name in [
            "locate_esm_rows.py",
            "build_esm_plan.py",
            "aws_locator_bootstrap.py",
            "pyproject.toml",
            "uv.lock",
        ]:
            archive.add(root / name, arcname=name)
    digest = hashlib.sha256(bundle.read_bytes()).hexdigest()
    source_key = OUTPUT_PREFIX + "/launches/" + args.run_name + "/source.tar.gz"
    download = (
...
/opt/exp292-bootstrap/bin/python /opt/exp292/aws_locator_bootstrap.py --bucket {shlex.quote(BUCKET)} --plan-prefix {shlex.quote(PLAN_PREFIX)} --output-prefix {shlex.quote(OUTPUT_PREFIX)}
"""
    ...
    record = {
        ...
        "plan": f"s3://{BUCKET}/{PLAN_PREFIX}",
        "results": f"s3://{BUCKET}/{OUTPUT_PREFIX}",
