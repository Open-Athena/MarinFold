...
from vocab import BEGIN_SEQUENCE, BEGIN_TOKENS, END, STRUCTURE_TOKEN  # noqa: E402
...
def test_generate_document_wellformed():
    ...
    assert f" {BEGIN_TOKENS} " in doc
    assert doc.endswith(f" {END}")
    # one code per atom; num_tokens = markers(4) + residues + atom codes.
    assert doc.count("<bt") == row["num_atoms"] == n_atom
    assert row["seq_length"] == n_res
    assert row["num_tokens"] == 4 + n_res + n_atom
