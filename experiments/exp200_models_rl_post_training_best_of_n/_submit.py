...
"""Shared iris submission for exp200 — issue #200.
...
exp200's own pinned manifest: marin 0.2.76 (the last release with ``marin.rl``)
...
def submit(
    *,
    ...
    if dry_run:
        print(f"[exp200] DRY RUN {job_name}\n  " + " ".join(argv))
        return job_name

    subprocess.run(argv, cwd=HERE, check=True)
    print(f"[exp200] submitted {job_name}")
...
def main_guard() -> None:
    if sys.version_info < (3, 12):
        raise SystemExit("exp200 targets python 3.12")
