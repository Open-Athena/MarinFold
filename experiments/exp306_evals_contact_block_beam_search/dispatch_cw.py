#!/usr/bin/env python
"""Dispatch independent single-H100 contact-block beam search shards."""
...
OUT = "s3://marin-us-east-02a/MarinFold/exp306/contact-block-beam-v1"
...
def bootstrap(shard: int, n_shards: int, args: argparse.Namespace) -> str:
    ...
    files = {
        "beam_worker_cw.py": HERE / "beam_worker_cw.py",
        "beam_policy.py": HERE / "beam_policy.py",
        "stage_model.py": HERE / "stage_model.py",
        "targets.csv": HERE / "data" / "targets.csv",
    }
    transfers = "\n".join(
        f"echo {encoded(path)} | base64 -d > /tmp/exp306/{name}"
    ...
    return f"""
set -euo pipefail
echo "[exp306] shard {shard}/{n_shards} host=$(hostname)"
...
"$VLLM_PY" /tmp/exp306/stage_model.py --source {shlex.quote(args.model)} --out /tmp/exp306-model
...
export PYTHONPATH=/tmp/exp306:${{PYTHONPATH:-}}
exec "$VLLM_PY" /tmp/exp306/beam_worker_cw.py \\
    --model /tmp/exp306-model \\
    --targets /tmp/exp306/targets.csv \\
    --out {shlex.quote(args.out)} \\
    --shard {shard}/{n_shards} \\
    --cohort {shlex.quote(args.cohort)} --split {shlex.quote(args.split)} \\
    --beam-width {args.beam_width} --n-rollouts {args.n_rollouts}{extra}
""".strip()
...
def request(shard: int, n_shards: int, args: argparse.Namespace) -> JobRequest:
    ...
    return JobRequest(
        name=f"exp306-{args.label}-s{shard}of{n_shards}",
...
def main() -> None:
    ...
    print(f"[exp306] {len(jobs)} H100 batch jobs; output={args.out}")
    if args.dry_run:
        for job in jobs:
    ...
    with open_iris_client(cluster_name="cw-rno2a", workspace=None) as iris_client:
        ...
        for job in jobs:
            ...
            print(f"[exp306] submitted {job.name} -> {getattr(submitted, 'id', submitted)}",
                  flush=True)
