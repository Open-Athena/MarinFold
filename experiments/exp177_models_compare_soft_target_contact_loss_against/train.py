...
class NextTokenDataKind(StrEnum):
    """Training data source for the next-token arm."""
...
def _next_token_train_component(
    *,
    data_kind: NextTokenDataKind,
    num_shards: int,
    examples_per_shard: int,
) -> tuple[DatasetComponent, bool | BlockShuffleConfig]:
...
def _identity_config(
    ...
    *,
    ...
    num_shards: int,
    examples_per_shard: int,
    next_token_data_kind: NextTokenDataKind,
) -> dict[str, object]:
    return {
        "name": name,
        ...
        "data": {
            ...
            "next_token_data_kind": next_token_data_kind.value,
            "transform_workers": int(os.environ.get("EXP177_TRANSFORM_WORKERS", "8")),
            "prefetch_shards": int(os.environ.get("EXP177_PREFETCH_SHARDS", "8")),
            "shard_cache_size": int(os.environ.get("EXP177_SHARD_CACHE_SIZE", "16")),
...
def build_step() -> ArtifactStep[LevanterCheckpoint]:
    ...
    next_token_data_kind = _next_token_data_kind()
    name = _run_name(loss_kind, steps, train_batch_size)
    if loss_kind == LossKind.NEXT_TOKEN and next_token_data_kind != NextTokenDataKind.CACHE and "EXP177_NAME" not in os.environ:
        name = f"{name}-{next_token_data_kind.value}"
    ...
    def build_config(ctx: StepContext) -> TrainLmOnPodConfig | dict[str, object]:
        identity = _identity_config(
            ...
            num_shards=num_shards,
            examples_per_shard=examples_per_shard,
            next_token_data_kind=next_token_data_kind,
        )
        ...
        if loss_kind == LossKind.SOFT_TARGET:
            ...
            train_component, training_shuffle = _next_token_train_component(
                data_kind=next_token_data_kind,
                num_shards=num_shards,
                examples_per_shard=examples_per_shard,
            )
        data = LmDataConfig(
