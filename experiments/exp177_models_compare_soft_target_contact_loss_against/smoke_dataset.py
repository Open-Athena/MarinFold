...
from premade_contacts_dataset import FixedQuotaPremadeContactsDataset, MPFixedQuotaPremadeContactsDataset
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--mp", action="store_true", help="Use the multiprocess shard-prefetch dataset")
    parser.add_argument("--transform-workers", type=int, default=2)
    ...
    dataset_cls = MPFixedQuotaPremadeContactsDataset if args.mp else FixedQuotaPremadeContactsDataset
    kwargs = {}
    if args.mp:
        kwargs = {
    ...
    if hasattr(dataset, "_slot_dataset"):
        stats["mp_queue"] = dataclasses.asdict(dataset._slot_dataset.stats)
    result = {
        "data_prefix": args.data_prefix,
        "num_shards": args.num_shards,
        "examples_per_shard": args.examples_per_shard,
