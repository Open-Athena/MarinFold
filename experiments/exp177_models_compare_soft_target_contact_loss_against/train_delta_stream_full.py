...
"""Train a Qwen-style LM on the full exp177 delta-stream contact cache."""
...
def run_train_job(config: TrainLmOnPodConfig) -> None:
    ...
    handle = current_client().submit(
        JobRequest(
            name=f"exp177-delta-stream-train-{uuid.uuid4().hex[:8]}",
...
def build_train_step(
    *,
    ...
    step = default_train(
        ...
        wandb_group="exp177-delta-stream-full",
        wandb_name=name,
