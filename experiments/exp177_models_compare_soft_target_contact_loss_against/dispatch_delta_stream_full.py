...
"""Dispatch exp177 delta-stream full next-token training on CoreWeave GPUs."""
...
PREFIX = "s3://marin-us-east-02a/protein-structure/MarinFold/exp177_contacts_delta_stream_v1"
CACHE_ROOT = os.environ.get("EXP177_CACHE_ROOT", f"{PREFIX}/packed_cache/2026.09.15.1")
VOCAB_SIZE = 2080
SEQ_LEN = 8192
...
def _resources() -> ResourceConfig:
    return ResourceConfig.with_gpu(
        os.environ.get("EXP177_CW_GPU_TYPE", "GB200"),
        count=int(os.environ.get("EXP177_CW_GPUS", "4")),
        replicas=int(os.environ.get("EXP177_CW_NODES", "2")),
        cpu=float(os.environ.get("EXP177_CW_CPU", "32")),
        ram=os.environ.get("EXP177_CW_RAM", "256GiB"),
        disk=os.environ.get("EXP177_CW_DISK", "256GiB"),
    )
...
def _optimizer() -> AdamConfig:
    return AdamConfig(
        learning_rate=float(os.environ.get("EXP177_LEARNING_RATE", "1e-3")),
        weight_decay=0.2,
        beta1=0.9,
        beta2=0.95,
        warmup=0.1,
...
def _pod_config(run_name: str, steps: int, resources: ResourceConfig) -> TrainLmOnPodConfig:
    ...
    env_vars = {
        ...
        "EXP177_CACHE_ROOT": CACHE_ROOT,
        "EXP177_DOCUMENT_STRUCTURE": "delta-stream-v1",
    ...
    steps_per_eval = int(os.environ.get("EXP177_CW_STEPS_PER_EVAL", "250"))
    keep_every_steps = int(os.environ.get("EXP177_CW_KEEP_EVERY_STEPS", "2000"))
    checkpoint_interval_minutes = int(os.environ.get("EXP177_CW_CHECKPOINT_INTERVAL_MINUTES", "10"))
    max_eval_batches = int(os.environ.get("EXP177_CW_MAX_EVAL_BATCHES", "2"))
    ...
    train_config = TrainLmConfig(
        ...
        trainer=TrainerConfig(
            ...
            tracker=WandbConfig(
                project="MarinFold",
                name=run_name,
                group="exp177-delta-stream-full",
                tags=["protein", "delta-stream", "contacts", "qwen3", "1_5b", "2x4gb200"],
                replicate_path=output_path,
            ),
            mp=jmp.get_policy("p=f32,c=bfloat16"),
            train_batch_size=int(os.environ.get("EXP177_CW_BATCH_SIZE", str(GLOBAL_BATCH_SIZE))),
...
def dispatch(wait: bool = True):
    steps = int(os.environ.get("EXP177_TRAIN_STEPS", str(DEFAULT_STEPS)))
    run_name = os.environ.get("EXP177_NAME") or os.environ.get("WANDB_NAME") or RUN_NAME
    ...
    request = JobRequest(
        ...
        priority=int(os.environ.get("EXP177_CW_PRIORITY_NUM", str(IRIS_PRIORITY_BAND_BATCH))),
        processes_per_task=1,
        max_retries_failure=int(os.environ.get("EXP177_CW_MAX_RETRIES", "3")),
    )
    logger.info("Dispatching exp177 delta-stream run %s -> %s", run_name, pod_config.output_path)
