...
"""Build fixed-length Levanter prebuilt caches for exp177 delta-stream training."""
...
DEFAULT_INPUT = (
    ...
    "exp177_contacts_delta_stream_v1/documents/2026.09.15.1/shard-*-of-03338.parquet"
)
DEFAULT_CACHE_ROOT = (
    "s3://marin-us-east-02a/protein-structure/MarinFold/"
    "exp177_contacts_delta_stream_v1/packed_cache/2026.09.15.1"
)
DEFAULT_META_ROOT = (
    "s3://marin-us-east-02a/protein-structure/MarinFold/"
    "exp177_contacts_delta_stream_v1/packed_cache_meta/2026.09.15.1"
)
VOCAB_SIZE = 2080
SEQ_LEN = 8192
PAD_TOKEN_ID = 0
...
def finalize_ledger(cache_split: str, meta_path: str) -> None:
    ...
    print(
        f"[exp177] finalized {cache_split}: {ledger.total_num_rows} examples, "
...
def build_split(
    *,
    ...
    ctx = ZephyrContext(
        ...
        name="exp177-delta-stream-packed-cache",
...
def main() -> None:
    ...
    parser.add_argument("--max-workers", type=int, default=int(os.environ.get("EXP177_PACKED_CACHE_MAX_WORKERS", "128")))
    ...
    train_paths = _list_inputs(args.input, max_shards=args.max_shards)
    val_paths = _list_inputs(args.input, max_shards=args.validation_shards)
    print(f"[exp177] train shards: {len(train_paths)} -> {args.cache_root}/train", flush=True)
    build_split(
        paths=train_paths,
    ...
    print(f"[exp177] validation shards: {len(val_paths)} -> {args.cache_root}/validation", flush=True)
    build_split(
        paths=val_paths,
