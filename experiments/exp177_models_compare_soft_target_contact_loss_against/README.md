...
marinfold_experiment:
  issue: 177
  title: "Compare soft-target contact losses against next-token contacts"
  kind: models
  branch: document-structures/contact-set-v1-tests
---

# Compare soft-target contact losses against next-token contacts
...
## Current direction: delta-stream contact documents

The first version of this experiment explored fixed/packed contact-target
representations (`contacts_set_v1`) with explicit contact slots. That made the
objective easy to define, but it pushed us toward a custom structured head and
risked expanding context length. The active formulation is now a compact
**delta-stream** document format trained with standard next-token cross entropy.
