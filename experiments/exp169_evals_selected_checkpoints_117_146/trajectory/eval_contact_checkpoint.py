...
"""Prepare and score one cataloged checkpoint with rollout contact voting.

The native Levanter checkpoint is restored from region-local GCS and converted
on ephemeral worker disk. Sparse votes and per-protein timings are committed to
the public HF bucket in restart-safe parts. A timing part is written last and
acts as the completion marker for the corresponding vote part.
"""
...
class Target:
    ...
    def key(self) -> tuple[str, str]:
        """Return the stable evaluation-unit key."""
...
def download_file(url: str, destination: Path) -> Path:
    """Stream one small public evaluation input to scratch."""
...
def recast_checkpoint(source: Path, destination: Path) -> int:
    ...
    excluded = {*shards, index_path.name}
    ...
    if index is not None:
        ...
        index_path = destination / index_path.name
        index_path.write_text(json.dumps(index, sort_keys=True))
    return total


def expected_geometry(spec: CheckpointSpec) -> dict[str, int]:
    """Return the HF config fields expected for a cataloged checkpoint."""

    geometry = spec.geometry
    expected = {
        "hidden_size": geometry.hidden_dim,
        "intermediate_size": geometry.intermediate_dim,
        "num_attention_heads": geometry.num_heads,
        "num_key_value_heads": geometry.num_kv_heads,
        "num_hidden_layers": geometry.num_layers,
        "vocab_size": 2845,
    }
    if geometry.head_dim is not None:
        expected["head_dim"] = geometry.head_dim
    return expected


def validate_checkpoint(path: Path, spec: CheckpointSpec) -> tuple[Any, Any]:
    """Require the cataloged geometry and dedicated contacts-v1 tokens."""

    config = AutoConfig.from_pretrained(path)
    tokenizer = AutoTokenizer.from_pretrained(path)
    expected = expected_geometry(spec)
    observed = {name: getattr(config, name, None) for name in expected}
    if observed != expected:
        raise ValueError(
            f"unexpected exported model geometry for {spec.key}: {observed}"
        )
    missing = []
...
def prepare_checkpoint(spec: CheckpointSpec, scratch: Path) -> tuple[Path, float]:
    """Export, cast, validate, and retain only the local BF16 model."""

    started = time.monotonic()
    destination = scratch / "prepared-bf16"
    marker = destination / ".complete.json"
    marker_value = {"checkpoint_uri": spec.checkpoint_uri, "step": spec.step}
    if marker.exists() and json.loads(marker.read_text()) == marker_value:
        validate_checkpoint(destination, spec)
        return destination, time.monotonic() - started

    source = scratch / "export-fp32"
    export_checkpoint(spec, source)
    total = recast_checkpoint(source, destination)
    config, tokenizer = validate_checkpoint(destination, spec)
...
class BucketOutput:
    ...
    def path(self, relative: str) -> str:
        """Return a path below this checkpoint's output prefix."""

        return f"{self.prefix}/{relative}"

    def ensure_manifest(self, manifest: dict[str, Any]) -> None:
        """Create the immutable run manifest or verify the existing one."""
    ...
    def part_complete(self, part: int, expected: Sequence[Target]) -> bool:
        """Return whether a complete and matching timing marker exists."""
    ...
    def upload(self, source: Path, relative: str) -> None:
        """Upload one local artifact with one connection."""
...
def timing_row(
    ...
    *,
    ...
    return {
        ...
        "runner_tag": "iris",
        "gpu_name": os.environ.get("MARINFOLD_ACCELERATOR", "unknown-tpu"),
...
def run(args: argparse.Namespace) -> int:
    ...
    manifest = {
        ...
        "iris_job": os.environ.get("MARINFOLD_IRIS_JOB"),
        "versions": {
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    """Parse one checkpoint evaluation invocation."""

    parser = argparse.ArgumentParser()
    parser.add_argument("--checkpoint", choices=sorted(CHECKPOINTS), required=True)
    parser.add_argument(
        "--scratch", type=Path, default=Path("/app/scratch/exp169-trajectory")
    )
