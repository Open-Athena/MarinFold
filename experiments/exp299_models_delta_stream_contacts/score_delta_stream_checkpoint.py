...
def gpu_metadata() -> dict[str, Any]:
    ...
    if gpu is None:
        return {"gpu_name": "", "gpu_total_memory_gb": "", "gpu_compute_capability": ""}
    return {
        "gpu_name": getattr(gpu, "device_kind", ""),
        "gpu_total_memory_gb": "",
...
def main() -> int:
    ...
    worker_meta = {
        "model_nickname": "delta-stream-step",
        "runner_tag": "iris",
        **gpu_metadata(),
    ...
    with trainer.use_device_mesh():
        ...
        print(f"[delta-score] scored={n_ok} skipped={n_skip} out={args.out_dir} timings={timings_out}", flush=True)
    return 0
