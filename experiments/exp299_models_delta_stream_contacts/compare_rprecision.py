...
def available_keys(records: list[dict[str, Any]], score_dir: str) -> set[tuple[str, str]]:
    keys: set[tuple[str, str]] = set()
    for rec in records:
        dataset = str(rec["dataset"])
        stem = str(rec["stem"])
        if score_exists(score_path(score_dir, dataset, stem)):
            keys.add((dataset, stem))
    return keys
...
def main() -> int:
    ...
    for spec in args.scores:
        ...
        score_specs.append((label, directory))

    common_keys: set[tuple[str, str]] | None = None
    for label, directory in score_specs:
        keys = available_keys(records, directory)
        print(f"[metrics] {label}: found scores for {len(keys)}/{len(records)} proteins")
        common_keys = keys if common_keys is None else common_keys & keys
    if common_keys is None:
        raise ValueError("no --scores entries provided")
    common_records = [rec for rec in records if (str(rec["dataset"]), str(rec["stem"])) in common_keys]
    print(f"[metrics] common evaluated proteins: {len(common_records)}/{len(records)}")

    rows: list[dict[str, Any]] = []
    for label, directory in score_specs:
        rows.extend(metric_rows(common_records, label, directory, args.score_key))
