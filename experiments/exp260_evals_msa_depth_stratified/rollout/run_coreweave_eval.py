...
"""Run the exp260 MSA-depth evaluation entirely inside CoreWeave.

The CPU driver builds the 887-unit union this experiment needs: all 333 scorable
FoldBench monomers (eval-val, eval-test, eval-denovo) plus the legacy 554, whose
CAMEO-hard and CASP-FM halves carry most of the shallow-MSA natural proteins. It
does not deduplicate overlapping PDB stems because evaluation identity is the
``(dataset, stem)`` pair.

Scoring is unchanged from PR #257 - same weights, same worker bytes, same
sampling recipe - so PR #257's legacy-554, eval-val, and eval-denovo aggregates
act as a reproduction gate on the new execution path before the previously
unscored eval-test slice is reported.
"""
...
def _read_jsonl(uri: str) -> list[dict]:
    """Read one small ground-truth JSONL file through fsspec."""

    with fsspec.open(uri, "rt") as handle:
        return [json.loads(line) for line in handle if line.strip()]


def _build_evaluation_union(
    *,
    legacy_targets_uri: str,
    legacy_ground_truth_uri: str,
    foldbench_targets_uri: str,
    foldbench_ground_truth_uri: str,
    foldbench_sets_uri: str,
    targets_uri: str,
    ground_truth_uri: str,
    subsets_uri: str,
) -> dict:
    """Build and validate the nondeduplicated 554 + 97 + 19 union."""

    legacy = _read_parquet(legacy_targets_uri)[["dataset", "stem", "L", "input_seq"]]
    foldbench = _read_parquet(foldbench_targets_uri)[
        ["dataset", "stem", "L", "input_seq"]
    ]
    with fsspec.open(foldbench_sets_uri, "rt") as handle:
        sets = pd.read_csv(handle)
    selected_sets = sets[(sets.scorable == 1) & sets.eval_set.isin(EVAL_SETS)]
    foldbench = foldbench[foldbench.stem.isin(selected_sets.stem)].copy()
    combined = pd.concat([legacy, foldbench], ignore_index=True)
    units = list(zip(combined.dataset, combined.stem, strict=True))

    foldbench_dataset = set(foldbench.dataset)
    if len(foldbench_dataset) != 1:
        raise ValueError(f"FoldBench dataset identity changed: {foldbench_dataset}")
    subsets = pd.concat(
        [
            legacy[["dataset", "stem"]].assign(subset="legacy_554"),
            selected_sets[["stem", "eval_set", "is_viral"]]
            .rename(columns={"eval_set": "subset"})
            .assign(dataset=next(iter(foldbench_dataset)))[
                ["dataset", "stem", "subset", "is_viral"]
            ],
        ],
        ignore_index=True,
    )
    subsets["is_viral"] = subsets.is_viral.astype("Int64")

    legacy_ground_truth = _read_jsonl(legacy_ground_truth_uri)
    foldbench_units = set(zip(foldbench.dataset, foldbench.stem, strict=True))
    foldbench_ground_truth = [
        record
        for record in _read_jsonl(foldbench_ground_truth_uri)
        if (record["dataset"], record["stem"]) in foldbench_units
    ]
    ground_truth = [*legacy_ground_truth, *foldbench_ground_truth]
    ground_truth_units = [
        (record["dataset"], record["stem"]) for record in ground_truth
    ]

    validation = {
        "units": len(units),
        "unique_units": len(set(units)),
        "unique_stems": len(set(combined.stem)),
        "ground_truth_units": len(ground_truth_units),
        "ground_truth_unique_units": len(set(ground_truth_units)),
        "subset_sizes": subsets.subset.value_counts().to_dict(),
        "overlapping_stems_retained": len(set(legacy.stem) & set(foldbench.stem)),
    }
    expected = {
        "units": EXPECTED_UNITS,
        "unique_units": EXPECTED_UNITS,
        "unique_stems": EXPECTED_UNIQUE_STEMS,
        "ground_truth_units": EXPECTED_UNITS,
        "ground_truth_unique_units": EXPECTED_UNITS,
        "subset_sizes": EXPECTED_SET_SIZES,
        "overlapping_stems_retained": EXPECTED_OVERLAPPING_STEMS,
    }
    if validation != expected:
        raise ValueError(f"evaluation union changed: {validation} != {expected}")
    if set(units) != set(ground_truth_units):
        raise ValueError("targets and ground truth cover different evaluation units")
    if (combined.input_seq.str.len() != combined.L).any():
        raise ValueError("target sequence lengths do not match L")

    _write_parquet(combined, targets_uri)
    with fsspec.open(ground_truth_uri, "wt") as handle:
        for record in ground_truth:
            handle.write(json.dumps(record, sort_keys=True) + "\n")
    with fsspec.open(subsets_uri, "wt") as handle:
        subsets.to_csv(handle, index=False)
    filesystem, destination = fsspec.core.url_to_fs(targets_uri)
    ...
    *,
    ...
    for model_slot, checkpoint in enumerate(checkpoints):
        ...
        for shard_idx in shards:
            name = f"e232t-{run_id}-{phase[:2]}-{checkpoint.job_label}-s{shard_idx:02d}"
...
def main() -> None:
    ...
    for checkpoint in checkpoints:
        ...
        verify_levanter_source(
            ...
            verification_uri=(
                f"{root}/inputs/checkpoint_verification/"
                f"{checkpoint.label}.levanter-source.json"
            ),
        )
        verify_checkpoint_at_uri(
            checkpoint=checkpoint,
    ...
    legacy_targets_uri = f"{root}/inputs/source/eval_targets_legacy.parquet"
    legacy_ground_truth_uri = f"{root}/inputs/source/gt_universe_legacy.jsonl"
    foldbench_targets_uri = f"{root}/inputs/source/eval_targets_foldbench.parquet"
    foldbench_ground_truth_uri = f"{root}/inputs/source/gt_universe_foldbench.jsonl"
    foldbench_sets_uri = f"{root}/inputs/source/eval_sets_foldbench.csv"
    targets_uri = f"{root}/inputs/eval_targets.parquet"
    ground_truth_uri = f"{root}/inputs/gt_universe.jsonl"
    subsets_uri = f"{root}/inputs/evaluation_subsets.csv"
    mirror_public_input(
    ...
    mirror_public_input(
        url=LEGACY_GROUND_TRUTH_URL,
        destination_uri=legacy_ground_truth_uri,
        expected_size=LEGACY_GROUND_TRUTH_SIZE,
        expected_sha256=LEGACY_GROUND_TRUTH_SHA256,
    )
    mirror_public_input(
        url=FOLDBENCH_TARGETS_URL,
        destination_uri=foldbench_targets_uri,
        expected_size=FOLDBENCH_TARGETS_SIZE,
        expected_sha256=FOLDBENCH_TARGETS_SHA256,
    )
    mirror_public_input(
        url=FOLDBENCH_GROUND_TRUTH_URL,
        destination_uri=foldbench_ground_truth_uri,
        expected_size=FOLDBENCH_GROUND_TRUTH_SIZE,
        expected_sha256=FOLDBENCH_GROUND_TRUTH_SHA256,
    )
    mirror_public_input(
        url=FOLDBENCH_SETS_URL,
        destination_uri=foldbench_sets_uri,
        expected_size=FOLDBENCH_SETS_SIZE,
        expected_sha256=FOLDBENCH_SETS_SHA256,
    )
    validation = _build_evaluation_union(
        legacy_targets_uri=legacy_targets_uri,
        legacy_ground_truth_uri=legacy_ground_truth_uri,
        foldbench_targets_uri=foldbench_targets_uri,
        foldbench_ground_truth_uri=foldbench_ground_truth_uri,
        foldbench_sets_uri=foldbench_sets_uri,
        targets_uri=targets_uri,
        ground_truth_uri=ground_truth_uri,
        subsets_uri=subsets_uri,
    )
    ...
    published = publish(run_root_uri=root, run_id=args.run_id)
    print(
        json.dumps(
            {"event": "complete", "result": result, "published": published},
            sort_keys=True,
        ),
        flush=True,
    )
