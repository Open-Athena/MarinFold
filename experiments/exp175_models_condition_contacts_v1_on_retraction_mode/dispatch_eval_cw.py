...
"""Generation-time retraction-mode comparison on CoreWeave (#175).

#160's ``dispatch_eval_cw.py`` with one dimension added: the two exp175 arms are
the **same checkpoint** prompted two ways —

* ``exp175-clean``        -> token 0 is ``<contacts-v1>``
* ``exp175-backtracking`` -> token 0 is ``<contacts-v1.backtracking>``

which is the comparison #160 could not make. Its ablation had to re-read a fixed
set of rollouts under two readouts; with a mode marker the model decides whether
to retract *while generating*, which is where the decision actually lives.

It ships **#160's worker unchanged** (exp169's precedent): running the same bytes
is what makes the numbers comparable across the two experiments. The worker's
``--backtracking-mode`` flag is off by default, so #160's results stay
reproducible byte-for-byte.

The CoreWeave venue is inherited from #160, where on 2026-07-28 the
...
S3_PREFIX = os.environ.get(
    "EVAL_CW_S3_PREFIX",
    "s3://marin-us-east-02a/protein-structure/MarinFold/exp175_backtracking_mode/eval")
...
JOB_PREFIX = os.environ.get("EVAL_CW_JOB_PREFIX", "exp175-eval-cw")

# label -> (model dir, prompt in retraction mode?). The two exp175 arms are the
# SAME checkpoint: only token 0 of the prompt differs. That is the whole point —
# #160 could only compare readouts of fixed rollouts, whereas a mode marker lets
...
WORKER_SCRIPT = (Path(__file__).resolve().parent.parent
                 ...
                 / "score_backtracking_worker.py")
WORK_DIR = "/tmp/exp175_eval_cw"
...
def build_bootstrap(*, label: str, model: str, mode: bool, shard_i: int,
                    num_shards: int, limit: int | None, commit: str) -> str:
    worker_b64 = base64.b64encode(WORKER_SCRIPT.read_bytes()).decode()
    limit_arg = f" --limit {limit}" if limit else ""
    mode_arg = " --backtracking-mode" if mode else ""
...
exec "$VLLM_PY" {WORKER_LOCAL} \\
    ...
    --top-p {TOP_P} \\
    --top-k {TOP_K} \\
    --contact-mult {CONTACT_MULT}{mode_arg}{limit_arg}
""".strip()


def build_request(*, label: str, model: str, mode: bool, shard_i: int,
                  num_shards: int, limit: int | None, commit: str,
                  name_suffix: str = "") -> JobRequest:
    return JobRequest(
        name=f"{JOB_PREFIX}-{label}-s{shard_i}of{num_shards}{name_suffix}",
        entrypoint=Entrypoint.from_binary(
            "bash", ["-lc", build_bootstrap(label=label, model=model, mode=mode,
...
def main() -> None:
    ...
    reqs = [build_request(label=lbl, model=ARMS[lbl][0], mode=ARMS[lbl][1], shard_i=i,
                          num_shards=a.num_shards, limit=a.limit, commit=commit,
