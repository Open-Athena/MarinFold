...
"""Per-row generation — fetch one structure, emit one contacts-and-crops-v1 document.

The *worker* half of the pipeline (adapted verbatim from exp105's ccoord
worker, swapping the format module). It deliberately has **no zephyr /
distributed import** so it can be unit-tested and run locally against the
manifest's ``gcs_uri`` column.

It calls **straight into** marinfold's contacts-and-crops-v1 generator
...
is fetched from the row's ``gcs_uri`` (the public AFDB bucket), parsed with
gemmi, and handed to ``generate_document`` with the manifest ``entry_id`` as
the deterministic seed. The output row is the format's ``metadata_row()``
plus the manifest provenance columns.
"""
...
def _load_rotamer_library() -> Any | None:
    ...
    Returns ``None`` on failure — ``generate_document`` then falls back to
    pyconfind's own lazy load (slower, but correct).
    """
    try:
...
def generate_shard(
    ...
    *,
    ...
    """Generate documents for one shard's rows.

    The rotamer library is loaded once for the whole shard. For the URI path,
    ...
    yield from thread_per_row_in_shard(
        ...
        thread_name_prefix="exp132-fetch",
    )
