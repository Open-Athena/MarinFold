...
"""Export an exp137 crops checkpoint to HF safetensors (CPU), tokenizer co-located.

Converts a levanter ``step-N`` checkpoint with the exp117 Qwen3 config + the
contacts-and-crops-v1 tokenizer co-located (hard rule: tokenizer travels with the
model). Full vocab (3848) is kept — the crops model's embedding covers all crops
tokens; no vocab slicing (unlike exp120's contacts-v1 export which sliced to 2845).

Runs locally on CPU in a marin/levanter venv (no TPU)::

    cd /home/bizon/git/marin
    uv run --no-sync python <exp137>/export_checkpoints.py \\
        --checkpoint gs://.../exp137_.../checkpoints/<run>/checkpoints/step-<N> \\
        --output-dir gs://.../exp137_.../hf/<run>/step-<N>
...
TOKENIZER_REPO = "timodonnell/contacts-and-crops-v1-tokenizer"
