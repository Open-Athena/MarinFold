...
Right panel: the paired per-protein differences the experiment preregistered --
``seeded - iid`` for consensus and for oracle best-of-100. Bars are means with
...
def deltas(axis, frame: pd.DataFrame) -> None:
    ...
    for position, (a, b, label) in enumerate(pairs):
        ...
        low, high = interval(d)
        mean = d.mean()
        axis.scatter(d, np.full(len(d), position) + np.random.default_rng(SEED)
                     .uniform(-0.16, 0.16, len(d)),
                     s=9, color=ARM_COLORS[a], alpha=0.35, zorder=2)
    ...
    axis.set_ylim(-0.6, len(pairs) - 0.15)
    axis.set_xlabel("Per-protein difference in R-precision (all ranges)")
    axis.grid(axis="x", color="#dddad6", linewidth=0.6)
    axis.set_axisbelow(True)
...
def main() -> int:
    ...
    figure, axes = plt.subplots(1, 2, figsize=(15.5, 6.4),
    ...
    stamp(dest, {"exp254_per_protein": args.data / "exp254_per_protein.csv.gz",
          ...
          "predictors, with the paired seeded-minus-i.i.d. differences.")
