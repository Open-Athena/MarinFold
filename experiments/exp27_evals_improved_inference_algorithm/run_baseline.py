...
"""Baseline driver: run the naive distogram readout on the 10 train proteins.

Spawns one persistent worker per visible CUDA GPU, each loading vLLM
once and pulling stems off a shared work queue. The 1B model is small
enough that 8 concurrent vLLM instances fit on 8 × V100-16GB
comfortably, and the work-stealing pool keeps idle GPUs busy when
fast proteins finish early.

The runtime headline number is **total wall-clock time** measured
around the whole pool — model loads + inference + queue overhead.
That's what the issue's 5× budget compares against.

After all 10 stems finish, the script runs ``score_marinfold.score_all``
against the new outputs and writes ``data/baseline_scores.csv`` + a
row appended to ``data/experiments.tsv``.

Idempotent: if a stem already has a matching ``provenance.json`` for
the requested ``algorithm`` name, it's left alone (and its
``elapsed_seconds`` is read back from the JSON so the TSV row still
makes sense). To force re-run, ``rm -r outputs/<stem>/``.
"""
...
def _worker(
    ...
    *,
    ...
    try:
        ...
        from naive_inference import load_runtime, predict_one
        rt = load_runtime(
...
def main() -> None:
    ...
    parser.add_argument(
        "--scores-out",
        type=Path,
        default=_THIS / "data" / "baseline_scores.csv",
    )
    parser.add_argument(
        "--experiments-tsv",
        type=Path,
    ...
    parser.add_argument("--algorithm", default="baseline_naive")
    parser.add_argument(
        "--description",
        default=(
            "Naive distogram readout (same as exp20): "
            "<begin_statements> with no seeded contacts, "
            "read top-128 logprobs per pair, renormalize over the 64 "
            "<d_X.X> bins."
        ),
    )
