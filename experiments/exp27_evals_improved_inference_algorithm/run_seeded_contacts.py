...
def _worker(
    ...
    *,
    ...
    try:
        ...
        from seeded_contacts_inference import load_runtime, predict_one
        rt = load_runtime(
    ...
    while True:
        ...
        try:
            elapsed = predict_one(
                ...
                order=order,
                prior_distogram_path=Path(out_dir) / stem / "distogram_baseline_naive.npz",
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--scores-out",
        type=Path,
        default=_THIS / "data" / "seeded_contacts_scores.csv",
    )
    parser.add_argument(
        "--experiments-tsv",
        type=Path,
    ...
    parser.add_argument("--algorithm", default="seeded_contacts_v1")
    parser.add_argument(
        "--description",
        default=(
            "Self-bootstrapped contact seeding. Seeds drawn from full "
            "baseline_naive distogram; readout on LDDT-shell pairs."
        ),
    )
