...
"""GT-filtered driver: same orchestration as ``run_baseline.py`` but
calls ``gt_filtered_inference.predict_one`` instead of the naive one.

Per-protein cost scales linearly with the number of pairs queried,
and we only query pairs where ``gt_d < 15 Å`` (LDDT inclusion radius
from ``score_marinfold``). Empirically that's 15% of pairs on this
train set → ~7× wall-clock speedup vs the naive baseline. The point
``lddt_distogram_cb`` column is mathematically unchanged (its
denominator is the same GT-shell mask); other score columns (MAE /
dRMSD / contact precision) become biased because they touch
out-of-shell pairs — treat them as invalid for this algorithm.

This is GT-aware: it cannot be run on held-out proteins. Its purpose
is to free wall-clock budget for richer inference algorithms.

Idempotent on (algorithm, distogram.npz) — re-run after any crash.
"""
...
def _worker(
    ...
    *,
    ...
    try:
        ...
        from gt_filtered_inference import load_runtime, predict_one
        rt = load_runtime(
...
def main() -> None:
    ...
    parser.add_argument(
        "--scores-out",
        type=Path,
        default=_THIS / "data" / "gt_filtered_scores.csv",
    )
    parser.add_argument(
        "--experiments-tsv",
        type=Path,
    ...
    parser.add_argument("--algorithm", default="gt_filtered_naive")
    parser.add_argument(
        "--description",
        default=(
            "Same readout as baseline_naive, restricted to (i, j) pairs "
            "where GT CB-CB distance < 15 A (LDDT inclusion radius). "
            "Point lddt_distogram_cb identical to baseline by "
            "construction; MAE/dRMSD/contact-precision columns are "
            "biased and should be ignored. GT-aware speedup intended "
            "to free wall-clock budget for richer algorithms."
        ),
    )
