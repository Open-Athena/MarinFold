...
"""Idea 6 driver: model-sampled contact prefix.

For each protein, sample N=``n_rollouts`` contact-statement prefixes
from the base prompt via ``sampled_contacts_inference.sample_contact_prefix``,
run a distance readout under each, and average the resulting
distograms. No naive prior required.
...
def _worker(
    ...
    *,
    ...
    try:
        ...
        from sampled_contacts_inference import load_runtime, predict_one
        rt = load_runtime(
...
def main() -> None:
    ...
    parser.add_argument(
        "--scores-out",
        type=Path,
        default=_THIS / "data" / "sampled_contacts_scores.csv",
    )
    parser.add_argument(
        "--experiments-tsv",
        type=Path,
    ...
    parser.add_argument("--algorithm", default="sampled_contacts_v1")
    parser.add_argument(
        "--description",
        default=(
            "Model-sampled contact prefix: temperature sampling from "
            "<begin_statements>, parse emitted contact statements, "
            "re-read distances at LDDT-shell pairs."
        ),
    )
