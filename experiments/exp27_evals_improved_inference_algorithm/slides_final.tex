...
\begin{frame}[plain]
  \begin{center}
    ...
    \begin{tabular}{@{}rl@{}}
      ...
      \large same algo on 1.5B (train): & {\Large\color{orange}\textbf{+9.04\%}} — \emph{does NOT transfer}\\
    \end{tabular}
    \\[1.2cm]
    {\small\color{navy}\codeS{iter\_R4\_grow\_on\_sampled\_uniform\_M5} —
      sampled-uniform contact prior + iterative growing-K seeding}\\[0.6cm]
...
\begin{frame}{Conclusion}
  \begin{itemize}
    ...
    \item \textbf{Closing this experiment.} Next directions worth
          chasing (not in scope here): re-probe + re-tune for 1.5B;
          extend iteration with cross-protein structural priors;
          replace post-hoc range-token uniform with a learned
          range-aware decoding head.
  \end{itemize}
\end{frame}
