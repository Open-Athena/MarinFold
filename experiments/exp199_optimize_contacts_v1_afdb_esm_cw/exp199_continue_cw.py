...
"""Continue a selected exp199 CoreWeave model from full trainer state.

The initial source is ``prot-exp199-cw-cv1-s02-m1-p06-aug`` at its permanent
``step-116160`` checkpoint, immediately before the source run's cooldown. The
continuation preserves model, AdamW, RNG, data-position, and absolute-step state,
holds the restored peak learning rate, and linearly cools down over its final 20%.

``SOURCE`` selects the source model. Only ``m1-p06-aug`` is currently supported;
the source catalog is deliberately structured to admit additional models later.
...
from exp199_sweep_cw import (
    EXPERIMENT_PREFIX as SWEEP_EXPERIMENT_PREFIX,
)

RUN_PREFIX = "prot-exp199-cw-cv1-cont"
CONTINUATION_EXPERIMENT_PREFIX = (
    "s3://marin-us-east-02a/marin/protein-structure/MarinFold/"
    "exp199_continue_contacts_v1_cw"
)

ORIGINAL_TRAIN_STEPS = 145_200
ADDITIONAL_TRAIN_STEPS = 3 * ORIGINAL_TRAIN_STEPS
MIN_LR_RATIO = 0.1
WARMUP = 0.1
REWARMUP = 0.0
DECAY = 0.2
...
SOURCE_MODELS = {
    "m1-p06-aug": SourceModel(
        key="m1-p06-aug",
        identity="srcaug",
        run_id="prot-exp199-cw-cv1-s02-m1-p06-aug",
        version="2026.08.07.2",
        checkpoint_step=116_160,
        mixture_key="m1",
        point_key="p06",
...
def _parse_placement(*, smoke: bool) -> tuple[str, ClusterSpec, int]:
    ...
    if not smoke:
        ...
        if nodes != required_nodes:
            raise SystemExit(
                "production continuation supports exactly two gang profiles: "
...
def source_checkpoint(source_model: SourceModel) -> ArtifactStep[LevanterCheckpoint]:
    ...
    return ArtifactStep[LevanterCheckpoint].adopt(
        (
            "checkpoints/protein/exp199-cw-continuation-source/"
