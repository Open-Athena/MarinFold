...
"""Loss-weight profiles over a contacts-v1 document -- numpy only.

Vendored VERBATIM from #163's ``loss_mask.py`` (only this paragraph is added).
exp230 uses the v2 ``span_weights`` half; ``answer_span_loss_weights`` and
``check_document_spans`` are kept so the file stays a byte-comparable copy and
the profile exp230 trains at is provably the one #163 measured.
