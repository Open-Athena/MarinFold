...
"""Gate A's scorer -- exp82's ``score_rollout_worker.py``, vendored VERBATIM.

**Do not edit the measurement.**  #209 established that exp82's worker is the
*reference* scorer for contacts-v1 R-precision: it is the only implementation
shown to agree with itself across accelerators and seeds (three runs span
0.0015), and it reads exp199 at 0.6103 where #199's own pipeline reads 0.5873.
Re-implementing it, or "improving" it, would make Gate A incomparable with every
number in #180's frontier table.

It is copied rather than imported because experiments do not import from each
other, and copied rather than adapted because a diff against exp82's file is the
cheapest possible proof that the measurement did not change.  exp230 supplies
only the inputs: the checkpoint, and the 554-unit target file built by
``build_eval_targets.py``.
