...
shared converted loss explicit. Each x tick shows the current loss above its
approximate historical equivalent in parentheses. The loss conversion affects
only those labels; all plotted R-precision values come from the original
per-protein evaluations. Lower loss runs to the right.
